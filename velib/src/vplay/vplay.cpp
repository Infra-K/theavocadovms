...
public:
	...
	VPlayData(VPlay &pPlay, bool bFile, bool bMJPEG, astring strFile, string strUser, string strPass,  BOOL bHWAccel)
	 : m_vPlay(pPlay), m_bFile(bFile), m_strFile(strFile), m_bMJPEG(bMJPEG), m_strUser(strUser), m_strPass(strPass), m_bHWAccel(bHWAccel)
	{
...
public:
	...
	static int CurlXferinfo(void *p, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow);
	int CurlXferinfo1(curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow);
...
void VPlayData::Run1()
{
	...
	while (m_bExit != true)
	{
		try {
		#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d Read File Source %s\n", __FUNCTION__, __FILE__, __LINE__, m_strFile.c_str());
		#endif
			ve_sleep(1000);	
		...
		#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d Read File Codec %d (%d, %d)\n", __FUNCTION__, __FILE__, __LINE__, m_strFile.c_str(), info.codec_id, info.width, info.height);
		#endif
		...
		#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d time diff %f %d\n", __FUNCTION__, __FILE__, __LINE__, videoTimeDiff1, videoTimeDiff);
		#endif
			while(m_bExit != true && deMuxer->read_frame( streamIndex ))
			{
			...
			#ifdef AVOCADO_DEBUG
				// VDC_DEBUG("%s %s %d current (%d, %d), videoTimeDiff %d\n", __FUNCTION__, __FILE__, __LINE__, current.tv_sec, current.tv_usec, videoTimeDiff);
			#endif
				...
				if (deMuxer->is_key() == true)
				{
					...
					memcpy(packet.dataBuf + sizeof(InfoFrameI), frame->map(), frame->get_data_size());
				...
				else
				{
					...
					memcpy(packet.dataBuf + sizeof(InfoFrameP), frame->map(), frame->get_data_size());
...
size_t VPlayData::MJPEGWriteData1(void *ptr, size_t size, size_t nmemb) 
{
	...
	VDC_DEBUG("%s %s %d got %d bytes (%d, %d)...\n", __FUNCTION__, __FILE__, __LINE__, nbytes, size, nmemb);
...
int VPlayData::CurlXferinfo(void *p, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow)
{
	int dummy = errno;
...
int VPlayData::CurlXferinfo1(curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow)
{
	if (m_bExit == true)
	{
		VDC_DEBUG("%s %s %d CurlXferinfo1 exit\n", __FUNCTION__, __FILE__, __LINE__);
		return 1;
	}
	else
	{
...
void VPlayData::RunMJPEG1()
{
	...
	{
		try {
			VDC_DEBUG("%s %s %d Read MJPEG Source %s\n", __FUNCTION__, __FILE__, __LINE__, m_strFile.c_str());
...
/* File video source */
BOOL VPlay::Init(string strFile, bool bMJPEG, string strUser, string strPass, BOOL bHWAccel)
{
	if (m_data)
	{
		delete m_data;
	...
	m_data->m_avWrapper.SetRawDataHandler((RMRawVideoHandler)(RenderWrapper::RenderRMRawVideoHandler), m_data->m_render);
	
	return TRUE;
}

BOOL VPlay::Init(BOOL bRealStream, string strUrl, string strUser, string strPass, BOOL bHWAccel, VSCConnectType connectType)
{
	if (m_data)
	{
		delete m_data;
	...
	m_data->m_avWrapper.Init(bRealStream, strUrl, strUser,strPass, bHWAccel, connectType);
	m_data->m_avWrapper.SetRawDataHandler(RMRawVideoHandler(RenderWrapper::RenderRMRawVideoHandler), m_data->m_render);
...
BOOL VPlay::GetLicenseInfo(astring &strHostId, int &ch, astring &type, astring &startTime, astring &expireTime)
{
...
size_t VPlay::EncodeJPEG(RawFrame& pRaw, int dst_w, int dst_h, u8* output, size_t outputSize)
{
	...
	AVKit::JPEGEncoder *pEnc = new AVKit::JPEGEncoder(AVKit::GetJPEGOptions(dst_w, dst_h));
	if (pEnc == NULL)
	{
		return -1;
	}
	...
	m_Scale.ScaleYUV420((unsigned char *)pRaw.data[0], pRaw.linesize[0], 
						...
						pRaw.width, pRaw.height, pRaw.width * 3, (unsigned char *)m_buf, dst_w, dst_h, dst_w * 3);
	size_t nRet = pEnc->EncodeYUV420P((uint8_t*)m_buf, (uint8_t*)(m_buf + dst_w* dst_h), (uint8_t*)(m_buf + ((dst_w * dst_h)* 5)/4), output, outputSize);
	delete pEnc;
	av_free(m_buf);
