...
BOOL HdfsRecSessionData::RegSeqCallback(HDFSDataHandler pCallback, void * pParam)
{
	m_handler = pCallback;
	m_pParam = pParam;
...
MFStatus HdfsRecSessionData::PushAFrame(VideoFrame *pFrame)
{
	...
	if (m_bGotInfoData != TRUE || m_infoData.video != CODEC_H264 || pFrame->streamType != VIDEO_STREAM_VIDEO)
	{
		// VDC_DEBUG("The Frame is not H264 or not Video data\n");
		VDC_DEBUG("%s %s %d The Frame is not H264 or not Video data\n", __FUNCTION__, __FILE__, __LINE__);
		return MF_OK;
	}
	...
	if (m_currStartTime == 0 && m_pMuxer == NULL)
	{
		if (m_bGotInfoData == FALSE)
		{
			...
			VDC_DEBUG("%s %s %d No InfoData\n", __FUNCTION__, __FILE__, __LINE__);
			return MF_OK;
		}
		StartNewRec(currTime);
	}

	if (currTime < m_currStartTime || m_currFrameCnt > 15 * 60 * 60 || currTime >= (m_currStartTime + m_inerval))
	{
		/* I Frame split file */
		if (pFrame->frameType == VIDEO_FRM_I)
		{
...
HdfsRecSession::HdfsRecSession(s32 deviceId, astring strName, VHdfsDB &pDB) : m_data(new HdfsRecSessionData(deviceId, strName, pDB))
{
...
BOOL HdfsRecSession::RegSeqCallback(HDFSDataHandler pCallback, void * pParam)
{
