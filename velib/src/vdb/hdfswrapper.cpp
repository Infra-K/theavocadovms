...
HdfsRecWrapper::HdfsRecWrapper(ck_string &pNameNode, ck_string &pPort, ck_string &pUser)
:m_hdfs(NULL), m_bConnected(FALSE), m_pNameNode(pNameNode), m_pPort(pPort), m_pUser(pUser)
{
	m_queue.start();
	Connecting();
...
BOOL HdfsRecWrapper::Connecting()
{
	...
	{
		m_hdfs = m_HdfslibDelegate.hdfsConnectAsUser(m_pNameNode.c_str(), m_pPort.to_uint16(10), m_pUser.c_str());
		if (m_hdfs == NULL)
		{
		...
		{
			VDC_DEBUG("%s Connect HDFS  successfully(%s,  %d).\n", __FUNCTION__, m_pNameNode.c_str(), m_pPort.to_uint16(10));
			m_bConnected = TRUE;
		}
	}
	else
	...
	switch (cmd.cmd)
	{
		case HDFS_CMD_FILE:
		{
			...
			if (m_bConnected == TRUE)
			{
				...
				hdfsFile writeFile = m_HdfslibDelegate.hdfsOpenFile(m_hdfs, strPath.c_str(), O_WRONLY, 0, 0, 0);
				if (writeFile != NULL)
				{
					tSize ret = m_HdfslibDelegate.hdfsWrite(m_hdfs, writeFile, (void *)(ptr.get_ptr()), ptr.length());
