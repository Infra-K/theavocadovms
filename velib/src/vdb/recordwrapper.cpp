...
/* A thread get the queue of the tcmalloc queue, and take care of the RecordSchedWeek use POCO DateTime.h to update the schedule. */
class RecordWrapperImpl 
{
public:
	RecordWrapperImpl(VDB &pVdb, astring deviceId, RecordSchedWeekMap cSchedMap, RecChangeFunctionPtr pCallback, void *pParam);
	~RecordWrapperImpl();
...
RecordWrapperImpl::RecordWrapperImpl(VDB &pVdb, astring deviceId, RecordSchedWeekMap cSchedMap, RecChangeFunctionPtr pCallback, void *pParam)
...
bool RecordWrapperImpl::ApplySched(std::time_t pTime)
{
	...
	if (nDayOfWeek < 0 || nDayOfWeek >= VDB_DAYS_IN_WEEK || nHourOfDay < 0 ||  nHourOfDay >= VDB_HOURS_IN_DAY)
	{
...
MFStatus RecordWrapperImpl::PushAFrame(VideoFrame &frame)
{
	...
	if ((pTime + 2)%3600 == 0 || m_SchedChanged == true)
	{
		ApplySched(pTime + 2);
	...
	if (m_nCurrRectype == R_OFF && m_bManualStart != true)
	{
		if (m_pRecord)
		{
			VDC_DEBUG("%s %s %d StopRecord()\n", __FUNCTION__, __FILE__, __LINE__);
			StopRecord();
		}
		// VDC_DEBUG("%s %s %d MF_OK\n", __FUNCTION__, __FILE__, __LINE__);
		return MF_OK;
	}
	if (m_pRecord == NULL)
	{
#ifdef AVOCADO_DEBUG
		// 녹화시 계속 호출됨
		// VDC_DEBUG("%s %s %d StartRecord() m_deviceId=%s, frame=%d\n", __FUNCTION__, __FILE__, __LINE__, m_deviceId.c_str(), (int)(frame.secs));
#endif
		m_pRecord = m_pVdb.StartRecord(m_deviceId, (int)(frame.secs), R_SCHED);

		m_pCallback(m_deviceId, true, m_pParam);
	}
	// VDC_DEBUG("Recording Size %d stream %d frame %d (%d, %d)\n", frame.dataLen, frame.streamType, frame.frameType, frame.secs, frame.msecs);	
	/* Just skip the info stream for recording */
	if (m_pRecord != NULL && frame.streamType != VIDEO_STREAM_INFO && m_pRecord->PushAFrame(&frame) == MF_WRTIE_REACH_END)
	{
		u32 endTime = m_pRecord->GetEndTime();
		if (endTime != 0)
		{
		...
		VDC_DEBUG("%s %s %d StartRecord() %s %d\n", __FUNCTION__, __FILE__, __LINE__, m_deviceId.c_str(), (int)(frame.secs));
		m_pRecord = m_pVdb.StartRecord(m_deviceId, (int)(frame.secs), R_SCHED);
		if (m_pRecord != NULL)
		{
...
RecordWrapper::RecordWrapper(VDB &pVdb, astring deviceId, RecordSchedWeekMap cSchedMap, RecChangeFunctionPtr pCallback, void *pParam)
								: m_pImpl(new RecordWrapperImpl(pVdb, deviceId, cSchedMap, pCallback, pParam))
{
