...
{
public:
	PlaybackWrapperImpl(VDB &pVdb, astring deviceId, u32 nPlaytime, PBCallbackFunctionPtr callback, BOOL eolLoop, void * pParam);
	~PlaybackWrapperImpl();
public:
...
PlaybackWrapperImpl::PlaybackWrapperImpl(VDB &pVdb, astring deviceId, u32 nPlaytime, PBCallbackFunctionPtr callback, BOOL eolLoop, void * pParam)
: m_pVdb(pVdb), m_deviceId(deviceId), m_nPlaytime(nPlaytime), m_callback(callback), m_pParam(pParam), m_pbSession(NULL), 
...
BOOL PlaybackWrapperImpl::FeedAFrame(VideoFrame &pbFrame, BOOL bWaiting)
{
	s32 nDataType;
	astring strPath;
	s32 nLastSec = 0;
	...
	if (m_pbSession)
	{
		if (m_pbSession->GetAFrame(pbFrame, waiting) == MF_WRTIE_REACH_END || pbFrame.secs == 0)
		{
#ifdef PB_WRAPPER_DEBUG
			VDC_DEBUG("%s %s %d Session reach end\n", __FUNCTION__, __FILE__, __LINE__);
#endif
			MakeUpSessionNext();
			waiting = 0;
		...
		if (bWaiting == TRUE)
		{
			...
			{
				VDC_DEBUG("%s %s %d waiting %d !!! \n", __FUNCTION__, __FILE__, __LINE__, waiting);
				// waiting = 5;
			}			
			ve_sleep(waiting);
#ifdef PB_WRAPPER_DEBUG
...
BOOL PlaybackWrapperImpl::MakeUpSessionNear(int seekTime)
{
	...
	{
#ifdef PB_WRAPPER_DEBUG
		VDC_DEBUG("%s %s %d Can not Search Playtime %d \n", __FUNCTION__, __FILE__, __LINE__, seekTime);
		ve_sleep(500);
#endif        	
	}
	if (m_pbSession == NULL)
	{
		if (m_pVdb.SearchAItemNear(m_deviceId, seekTime, m_recordItem) == TRUE)
		{
			...
			{
#ifdef PB_WRAPPER_DEBUG
				VDC_DEBUG("%s %s %d Create new pb Near session %s \n", __FUNCTION__, __FILE__, __LINE__, strPath.c_str());
#endif
				m_pbSession = new PlaybackSession(m_deviceId, strPath, TRUE);
			}
		}
		...
		{
#ifdef PB_WRAPPER_DEBUG
			VDC_DEBUG("%s %s %d Can not Search Near Playtime %d \n", __FUNCTION__, __FILE__, __LINE__, seekTime);
#endif        
			ve_sleep(500);
			m_gotEol = TRUE;
...
{
#ifdef PB_WRAPPER_DEBUG
	VDC_DEBUG("%s %s %d Playtime %d \n", __FUNCTION__, __FILE__, __LINE__, m_nPlaytime);
#endif
...
PlaybackWrapper::PlaybackWrapper(VDB &pVdb, astring deviceId, u32 nPlaytime, PBCallbackFunctionPtr callback, BOOL eolLoop, void * pParam)
: m_pImpl(new PlaybackWrapperImpl(pVdb, deviceId, nPlaytime, callback, eolLoop, pParam))
{
