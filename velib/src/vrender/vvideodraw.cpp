...
bool VVideoDraw::Init(int w, int h)
{
	...
	snprintf(args, sizeof(args), "video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:pixel_aspect=%d/%d", m_width,m_height,AV_PIX_FMT_YUV420P, 1, 100, 1, 1);
	ret = avfilter_graph_create_filter(&m_buffersrc_ctx, m_buffersrc, "in",	args, NULL, m_filter_graph);
	if (ret < 0) {
	...
	ret = avfilter_graph_create_filter(&m_buffersink_ctx, m_buffersink, "out", NULL, m_buffersink_params, m_filter_graph);
	...
	m_inputs->pad_idx    = 0;
	m_inputs->next       = NULL;
	if ((ret = avfilter_graph_parse_ptr(m_filter_graph, filter_descr, &m_inputs, &m_outputs, NULL)) < 0)
	{
	...
	av_image_fill_arrays(m_frame_in->data, m_frame_in->linesize,m_frame_buffer_in, AV_PIX_FMT_YUV420P,m_width, m_height,1);	
	...
	av_image_fill_arrays(m_frame_out->data, m_frame_out->linesize,m_frame_buffer_out, AV_PIX_FMT_YUV420P,m_width, m_height,1);
