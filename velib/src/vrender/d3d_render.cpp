...
bool d3d_render::render_one_frame(RenderFrame *data, int pix_fmt)
{
	...
	{
		printf("VIDEO_HW_NONE unlock failed.\n");
	...
	if (m_bHWAccel == true && data->hwCmd == VIDEO_HW_GET_D3D_DEV)
	{
		...
		printf("VIDEO_HW_GET_D3D_DEV unlock failed.\n");

		m_Lock.unlock();
		return true;
	}

	if (m_image_width != data->width || m_image_height != data->height || m_last_error == true)
	{
		...
		// printf("VIDEO_HW_NONE.\n");
	}

	m_last_error = false;
	HRESULT hr;
	char *src_yuv[3] = { data->data[0], data->data[2], data->data[1] };

	int src_linesize[3] = { data->linesize[0], data->linesize[2], data->linesize[1] };
	...
	if (!m_d3d_device)
	{
		printf("m_d3d_device unlock failed.\n");
		m_Lock.unlock();
		return false;
	}
	...
	if (data->hwCmd == VIDEO_HW_NONE)
	{
		if ((rect_win.right - rect_win.left) != (m_last_client_rect.right - m_last_client_rect.left) ||
			(rect_win.bottom - rect_win.top) != (m_last_client_rect.bottom - m_last_client_rect.top))
		{
			...
			{
				printf("create_d3d_surfaces unlock failed.\n");
				m_Lock.unlock();
				return 0;
			}
		}
		...
		//printf("VIDEO_HW_NONE.\n");
	//}	
	//if (data->hwCmd == VIDEO_HW_NONE)//infrak
	...
	//{
		...
		{
			printf("pBits && m_d3d_surface unlock failed.\n");
			m_Lock.unlock();
			return false;
		}
		...
		int dst_linesize[3] = { m_locked_rect.Pitch, m_locked_rect.Pitch >> 1, m_locked_rect.Pitch >> 1 };
#ifdef DISABLE_COVER
	    ...
	    for (unsigned n = 0; n < 3; n++) {
	        ...
	        cover_SSE_CopyPlane(dst_yuv[n], dst_linesize[n], (unsigned char *)src_yuv[n], src_linesize[n], m_pCache.buffer, m_pCache.size,
	...
	{
		hr = m_d3d_device->StretchRect(m_d3d_surface, NULL, m_d3d_backbuf, NULL, D3DTEXF_LINEAR);		
	}
	...
	if (FAILED(hr))
	{
		...
		case D3DERR_DRIVERINTERNALERROR:
			printf("%s %s %d D3DERR_DRIVERINTERNALERROR\n", __FUNCTION__, __FILE__, __LINE__);
			break;
		case D3DERR_DEVICELOST:
			printf("%s %s %d D3DERR_DEVICELOST\n", __FUNCTION__, __FILE__, __LINE__); // 이것이 많이 나옴
			break;
		case D3DERR_INVALIDCALL:
			printf("%s %s %d D3DERR_INVALIDCALL\n", __FUNCTION__, __FILE__, __LINE__);
			break;
		default:
			break;
		}
		//printf("Present failed.\n");
		printf("%s %s %d Present failed.\n", __FUNCTION__, __FILE__, __LINE__);
		 */
		/*
		if (hr == D3DERR_DEVICELOST)
		{
			m_d3d_device->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_COLORVALUE(0.0f, 1.0f, 0.0f, 1.0f), 1.0f, 0);
...
bool d3d_render::configure_d3d()
{
	...
	if (m_d3d_handle)
	{
		hr = m_d3d_handle->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hwnd, m_BehaviorFlags, &m_present_params, &m_d3d_device);
		if (FAILED(hr))
		{
...
bool d3d_render::create_d3d_surfaces()
{
	...
	if (!m_d3d_surface)
	{
		hr = m_d3d_device->CreateOffscreenPlainSurface(m_image_width, m_image_height, m_movie_src_fmt, D3DPOOL_DEFAULT, &m_d3d_surface, NULL);
		if (FAILED(hr))
		{
...
int d3d_render::query_format(uint32_t movie_fmt)
{
	...
	for (i = 0; i < DISPLAY_FORMAT_TABLE_ENTRIES; i++)
	{
		if (fmt_table[i].mplayer_fmt == movie_fmt)
		{
			...
			hr = m_d3d_handle->CheckDeviceFormatConversion(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, fmt_table[i].fourcc, m_desktop_fmt);
			if (FAILED(hr))
			{
