...
public:
	...
	: m_bInited(FALSE), m_pData(NULL), m_rawFrameHandler(NULL), m_bHWAccel(bHWAccel)
    {
    	gRenderNoVideoLock.lock();
		if (gRenderNoVideoPic == NULL)
		{
			shared_ptr<jpeg_decoder> decoder = make_shared<jpeg_decoder>(get_jpeg_options(640, 480, 1, 1, 255));
		...
		memcpy(m_frameCache.data[1], gRenderNoVideoPic->map() + m_frameCache.width * m_frameCache.height, (m_frameCache.width * m_frameCache.height)/4);

		m_frameCache.data[2] = (char *)malloc ((m_frameCache.width * m_frameCache.height)/4);
		m_frameCache.linesize[2] = m_frameCache.width /2;
		memcpy(m_frameCache.data[2], gRenderNoVideoPic->map() + ((m_frameCache.width * m_frameCache.height)* 5/4), (m_frameCache.width * m_frameCache.height)/4);
		m_bExit = false;
