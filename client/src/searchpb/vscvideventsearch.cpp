 ...
 : m_pFactory(pFactory), m_pVideo(NULL), m_pCamTree(pCamTree), QWidget(parent, flags)
{
	...
	connect(&(m_pFactory.GetStorFactory()), SIGNAL(SignalEvent(std::string)), this, SLOT(SlotEvent(std::string)));
	...
	connect(ui.startTime, SIGNAL(dateTimeChanged(QDateTime)), this, SLOT(SlotStartDateTimeChanged(QDateTime)));
...
void VSCVidEventSearch::SlotSectionClicked(int row, int column)
{
	...
	if (pEvent )
	{
		...
		if (cEvent.bhandled() != true)
		{
			...
			m_pFactory.GetStorFactory().HandleEvent(cEvent.strstorid(), cEvent.strid());
...
void VSCVidEventSearch::SlotSearch()
{
	...
	{
		m_pFactory.GetStorFactory().SearchEvent(strStorId, strId, nStartTime, nEndTime);
	}
}
