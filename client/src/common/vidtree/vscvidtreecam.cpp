...
void VSCVidTreeCam::Init()
{
	...
	m_pRoot->setText(0, QApplication::translate("Camera", "Camera", 0));	
...
void VSCVidTreeCam::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (selectedItem)
	{
		...
		if (pCam)
		{
			VSCVidItemVidStor *pStor = dynamic_cast<VSCVidItemVidStor *> (pCam->QTreeWidgetItem::parent());
			if (pStor)
			{
#ifdef AVOCADO_DEBUG
				VDC_DEBUG("%s %s %d id Stor %s Camera %s \n", __FUNCTION__, __FILE__, __LINE__, pStor->GetId().c_str(), pCam->GetId().c_str());
#endif
				QMimeData *mimeData = new VSCQMimeVidCam(pStor->GetId(), pCam->GetId(), pCam->GetName());
...
void VSCVidTreeCam::mouseDoubleClickEvent(QMouseEvent *event)
{
	...
	if (selectedItem)
	{
		...
		if (pCam)
		{
			...
			{
#ifdef AVOCADO_DEBUG
				VDC_DEBUG( "%s id Stor %s Camera %s \n",__FUNCTION__, pStor->GetId().c_str(), pCam->GetId().c_str());
#endif
...
void VSCVidTreeCam::TreeUpdate()
{
	...
	for (s32 i = 0; i < storList.cvidstor_size(); i ++)
	{
		...
		VSCVidItemVidStor *pItemStor = new  VSCVidItemVidStor(pStor, m_pFactory, m_pRoot);
	}
