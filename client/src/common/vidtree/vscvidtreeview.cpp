...
void VSCVidTreeView::Init()
{
	...
	m_pRoot->setText(0, QApplication::translate("View", "View", 0));
...
{
	// Views 를 열어서 트리에 나타나는 저장 화면을 우측에 나타냄
	QTreeWidgetItem *selectedItem = currentItem();
...
void VSCVidTreeView::TreeUpdate()
{
	...
	for (s32 i = 0; i < viewList.cvidview_size(); i++)
	{
		...
		VSCVidItemView *pItemView = new VSCVidItemView(pView, m_pFactory, m_pRoot);
	}	
}
...
#ifdef AVOCADO_DEBUG
	VDC_DEBUG("%s %s %d cnt=%d\n", __FUNCTION__, __FILE__, __LINE__, cnt);
#endif
}
