...
VSCVidList::VSCVidList(QWidget *parent)
 : QWidget(parent), m_pVidInf(NULL), m_pCameraTree(NULL), m_pGroupTree(NULL), m_pViewTree(NULL), m_pEmapTree(NULL), m_pConfTree(NULL)
{
	ui.setupUi(this);
	...
	connect(ui.filterText, SIGNAL(textChanged(const QString &)), this, SLOT(FilterChanged(const QString &)));
