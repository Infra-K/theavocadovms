 ...
 : QWidget(parent)
{
	...
	ui.pbAlarm->setToolTip(tr("You can see the camera's event console video screen.")); //infrak
...
void VSCToolBar::showAlarm()
{
	if (m_alarmCnt <= 0)
	{
		...
		ui.pbAlarm->setIcon(QIcon(":/action/resources/alarmno.png"));
#ifdef AVOCADO_DEBUG
		VDC_DEBUG("%s %s %d, alarmno 0, m_alarmCnt=%d\n", __FUNCTION__, __FILE__, __LINE__, m_alarmCnt);
#endif
	}
	else
	{
		if (m_alarm == false)
		{
			...
			m_alarm = true;
#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d, alarmno, m_alarmCnt=%d\n", __FUNCTION__, __FILE__, __LINE__, m_alarmCnt);
#endif
		}
		else
		{
			...
			m_alarm = false;
#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d, alarm, m_alarmCnt=%d\n", __FUNCTION__, __FILE__, __LINE__, m_alarmCnt);
#endif
		}
		m_alarmCnt--;
	}
...
bool VSCToolBar::event(QEvent *e)
{
	//#ifdef AVOCADO_DEBUG
	// VDC_DEBUG("%s Event Type %d\n", __FUNCTION__, e->type());
	// VSCView::event Event Type 4
	//#endif
...
void VSCToolBar::mouseDoubleClickEvent(QMouseEvent *e)
{
