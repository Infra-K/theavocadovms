...
VidCamAdd::VidCamAdd(VidStor &pStor, ClientFactory &pFactory, QWidget *parent, Qt::WindowFlags flags)
 : m_sStor(pStor), m_Timer(NULL), m_pFactory(pFactory), m_bStarted(false), m_syncInfSearch(NULL), m_bSelectedAll(false), QWidget(parent, flags)
{
	ui.setupUi(this);
	setAcceptDrops(true);
...
void VidCamAdd::SlotRadioButtonClicked()
{
	...
	if(this->ui.radioButtonRtsp->isChecked() || this->ui.radioButtonMJPEG->isChecked())
	{
...
bool VidCamAdd::CheckIPPort(s8 * ipAddr, s8 * Port)
{
	...
	s8 rowip[1024];
	s8 rowPort[1024];	
	// VDC_DEBUG("%s %s %d Searched=%d\n", __FUNCTION__, __FILE__, __LINE__, insertRow);
...
{
	// ip 또는 port의 정보를 테이블 내부에서 정보 가져오기
	// VDC_DEBUG("%s %s %d\n", __FUNCTION__, __FILE__, __LINE__);
    if (pParam && item)
    {
...
void VidCamAdd::SlotSearchRecv()
{
	...
	while (bGotCam)
	{
		...
		else
		{
			...
			{
				insertRow = ui.tableSearch->rowCount();
#ifdef AVOCADO_DEBUG
				VDC_DEBUG("AddItem === > %s, insertRow:%d\n", strIP.c_str(), insertRow);
#endif
				ui.tableSearch->insertRow(insertRow);
				QTableWidgetItem *firstCheck = new QTableWidgetItem("");
				firstCheck->setCheckState(Qt::Unchecked);
...
void VidCamAdd::SlotStartSearch()
{
	...
	else
	{
		...
		checked = m_syncInfSearch->Connect();
		if (checked)
		{
			checked = m_syncInfSearch->CamSearchStart();
			if (checked)
			{
				m_Timer = new QTimer();
...
void VidCamAdd::SlotAddAll()
{
	...
	for (int i = 0; i < insertRow; i ++)
	{
		...
		{
#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d strip: %s\n", __FUNCTION__, __FILE__, __LINE__, ui.tableSearch->item(i, 1)->text().toStdString());
#endif
			continue;
		}
