...
LinkServerPlayback::LinkServerPlayback(LinkPlaybackInterface &pPbInf, Factory &pFactory, astring strId, u32 nPlaytime)
 : m_pFactory(pFactory), m_pPbInf(pPbInf), m_strId(strId), m_nPlaytime(nPlaytime), m_pbWrapper(pFactory.GetVdb(), strId, nPlaytime,
 (PBCallbackFunctionPtr)(LinkServerPlayback::DataHandler), false, this), m_bQuit(false), m_pThread(NULL)
{
...
bool LinkStream::ProcessLoginReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
...
bool LinkStream::ProcessStartLiveReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
	long long p = (long long)conn;
	Link::LinkCmd cmdResp;
	...
	if (m_nStream == 1)
	{
		m_pFactory.RegDataCallback(pReq.strid(), (CameraDataCallbackFunctionPtr)LinkStream::DataHandler, (void *)this);
	}
	else if (m_nStream == 2)
	{
		m_pFactory.RegSubDataCallback(pReq.strid(),	(CameraDataCallbackFunctionPtr)LinkStream::DataHandler, (void *)this);
	}
	else 
	{
		m_pFactory.RegDataCallback(pReq.strid(), (CameraDataCallbackFunctionPtr)LinkStream::DataHandler, (void *)this);
...
bool LinkStream::ProcessStopLiveReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
	long long p = (long long)conn;
	Link::LinkCmd cmdResp;
...
bool LinkStream::ProcessPlayBackReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
	long long p = (long long)conn;
	Link::LinkCmd cmdResp;
	...
	m_pPlayback = new LinkServerPlayback(*this, m_pFactory, pReq.strid(), pReq.nplaytime());
...
bool LinkStream::ProcessPlayPauseReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
	long long p = (long long)conn;
	Link::LinkCmd cmdResp;
...
bool LinkStream::ProcessPlayResumeReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
	long long p = (long long)conn;
	Link::LinkCmd cmdResp;
...
bool LinkStream::ProcessPlaySeekReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
	long long p = (long long)conn;
	Link::LinkCmd cmdResp;
...
bool LinkStream::ProcessPlayStopReq(Link::LinkCmd &req, CivetServer *server, struct mg_connection *conn)
{
	long long p = (long long)conn;
	Link::LinkCmd cmdResp;
...
void LinkStream::DataHandler1(VideoFrame& frame)
{
	...
	int ret = mg_websocket_write(m_conn, WEBSOCKET_OPCODE_BINARY, (const char *)m_pSendBuf, frame.dataLen + sizeof(VideoFrameHeader));
...
bool LinkStream::ProcessMsg(std::string &strMsg, CivetServer *server, struct mg_connection *conn)
{
	Link::LinkCmd cmd;
...
bool LinkStream::SendRespMsg(Link::LinkCmd &resp, CivetServer *server, struct mg_connection *conn)
{
	std::string strMsg;
