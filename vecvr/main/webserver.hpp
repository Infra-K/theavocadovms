...
{
public:
	VEWebServer(std::vector<std::string> cpp_options, Factory &pFactory, VEventServer &pEvent)
	 : pServer(new CivetServer(cpp_options)), server(*pServer), m_pFactory(pFactory), h_GetCamList(pFactory), h_GetImage(pFactory), h_vwsapi(pFactory), 
	h_wslink(pFactory, pEvent), h_wslinkStream(pFactory), h_GetStreamUrl(pFactory)
	{
		/* restful api */
