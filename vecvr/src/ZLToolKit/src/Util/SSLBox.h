...
public:
	...
	//收到密文后，调用此函数解密 //암호문을 수신 한 후이 함수를 호출하여 해독합니다.
	void onRecv(const char *data, uint32_t data_len);
	//需要加密明文调用此函数 //이 함수를 호출하려면 일반 텍스트를 암호화해야합니다.
	void onSend(const char *data, uint32_t data_len);

	//设置解密后获取明文的回调 //복호화 후 일반 텍스트를 가져 오도록 콜백 설정
	template<typename F>
	void setOnDecData(F &&fun) {
		onDec = fun;
	}

	//设置加密后获取密文的回调 //암호화 후 암호문을 가져 오도록 콜백 설정
	template<typename F>
