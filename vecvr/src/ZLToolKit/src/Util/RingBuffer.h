...
//实现了一个一写多读得环形缓冲的模板类 //하나의 쓰기 및 다중 읽기 링 버퍼가있는 템플릿 클래스 구현
...
public:
	...
	public:
		...
		//重新定位读取器至最新的数据位置 //판독기를 최신 데이터 위치로 재배치
		void reset(bool keypos = true) {
			auto strongBuffer = buffer.lock();
			if (strongBuffer) {
				if(keypos){
					curpos = strongBuffer->ringKeyPos; //定位读取器 //위치 리더
				}else{
	...
	private:
		...
		const T *read(RingBuffer *ring) {
			...
			const T *data = &(ring->dataRing[curpos]); //返回包 //반품 패키지
			curpos = ring->next(curpos); //更新位置 //위치 업데이트
			return data;
		}
	...
	//写环形缓冲，非线程安全的 //스레드로부터 안전하지 않은 쓰기 링 버퍼
	void write(const T &in,bool isKey = true) {
		computeBestSize(isKey);
        dataRing[ringPos] = in;
        if (isKey) {
            ringKeyPos = ringPos; //设置读取器可以定位的点 //독자가 찾을 수있는 지점 설정
        }
...
private:
	...
	int ringKeyPos;
	int ringSize;
	//计算最佳环形缓存大小的参数 //최적의 링 버퍼 크기를 계산하기위한 매개 변수
	int besetSize = 0;
	int totalCnt = 0;
	int lastKeyCnt = 0;
	...
	void computeBestSize(bool isKey){
		...
		//关键帧 //키 프레임
		if(lastKeyCnt){
			//计算两个I帧之间的包个数 //두 I 프레임 사이의 패킷 수 계산
