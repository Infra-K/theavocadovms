...
void Socket::connect(const string &url, uint16_t port, onErrCB &&connectCB,
	...
	if(result == -1){
		WarnL << "开始Poll监听失败";//폴링 모니터링 시작 실패
		SockException err(Err_other,"开始Poll监听失败");//폴링 모니터링 시작 실패
		connectCB(err);
		return;
	}
...
void Socket::onConnected(const SockFD::Ptr &pSock,const onErrCB &connectCB) {
	...
	if (!err) {
		...
		if(!attachEvent(pSock,false)){
			WarnL << "开始Poll监听失败";//폴링 모니터링 시작 실패
			err.reset(Err_other, "开始Poll监听失败");//폴링 모니터링 시작 실패
			goto failed;
		}
		pSock->setConnected();
		connectCB(err);
	...
	while (true) {
		if (event & Event_Read) {
			...
			if (peerfd == -1) {
				...
				ErrorL << "tcp服务器监听异常:" << strerror(errno);//tcp 서버 모니터링 예외 :
				onError(pSock);
				return -1;
			}
		...
		if (event & Event_Error) {
			ErrorL << "tcp服务器监听异常:" << strerror(errno);//TCP 서버가 비정상적으로 수신합니다.
			onError(pSock);
			return -1;
		}
	}
...
bool Socket::setPeerSock(int sock, struct sockaddr *addr) {
	...
	auto pSock = makeSock(sock);
	if(!attachEvent(pSock,false)){
		WarnL << "开始Poll监听失败";//폴링 모니터링을 시작하지 못했습니다.
		return false;
	}
	_peerAddr = *addr;
	lock_guard<spin_mutex> lck(_mtx_sockFd);
...
void Socket::enableRecv(bool enabled) {
	...
	_enableRecv = enabled;
	if(!enabled){
		//关闭套接字接收事件 //이벤트 수신을위한 소켓 닫기
		return;
	}
	//打开套接字接收事件 //이벤트를 받을 소켓을 엽니다.
	weak_ptr<Socket> weakSelf = this->shared_from_this();
	ASYNC_TRACE([weakSelf]() {
		...
		if(sock){
			//触发读事件，边沿触发才有意义 //트리거 읽기 이벤트, 에지 트리거가 의미 있음
