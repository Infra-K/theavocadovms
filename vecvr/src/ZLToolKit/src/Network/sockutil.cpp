...
int SockUtil::setCloseWait(int sockFd, int second) {
	linger m_sLinger;
	//在调用closesocket()时还有数据未发送完，允许等待 // closesocket () 호출시 아직 전송되지 않은 데이터가 있으며 대기 할 수 있습니다.
	// 若m_sLinger.l_onoff=0;则调用closesocket()后强制关闭 // m_sLinger.l_onoff = 0;이면 closesocket ()이 강제로 닫힙니다.
	m_sLinger.l_onoff = (second > 0);
	m_sLinger.l_linger = second; //设置等待时间为x秒 // 대기 시간을 x 초로 설정
	int ret = setsockopt(sockFd, SOL_SOCKET, SO_LINGER,
			(const char*) &m_sLinger, sizeof(linger));
	if (ret == -1) {
		TraceL << "设置 SO_LINGER 失败！";//SO_LINGER를 설정하지 못했습니다!
	}
	return ret;
}
int SockUtil::setNoDelay(int sockFd, bool on) {
	...
	if (ret == -1) {
		TraceL << "设置 NoDelay 失败！";//NoDelay를 설정하지 못했습니다!
	}
	return ret;
}
...
int SockUtil::setReuseable(int sockFd, bool on) {
	...
	if (ret == -1) {
		TraceL << "设置 SO_REUSEADDR 失败！";//SO_REUSEADDR을 설정하지 못했습니다!
	}
	return ret;
}
int SockUtil::setBroadcast(int sockFd, bool on) {
	...
	int ret = setsockopt(sockFd, SOL_SOCKET, SO_BROADCAST, &opt,
			static_cast<socklen_t>(sizeof(opt)));
	if (ret == -1) {
		TraceL << "设置 SO_BROADCAST 失败！";//SO_BROADCAST를 설정하지 못했습니다!
	}
	return ret;
}
...
int SockUtil::setKeepAlive(int sockFd, bool on) {
	...
	int ret = setsockopt(sockFd, SOL_SOCKET, SO_KEEPALIVE, &opt,
			static_cast<socklen_t>(sizeof(opt)));
	if (ret == -1) {
		TraceL << "设置 SO_KEEPALIVE 失败！";//SO_KEEPALIVE를 설정하지 못했습니다!
	}
	return ret;
}
...
int SockUtil::setNoSigpipe(int sd) {
	...
	if (ret == -1) {
		TraceL << "设置 SO_NOSIGPIPE 失败！";//SO_NOSIGPIPE를 설정하지 못했습니다!
	}
#endif
	return ret;
}
...
int SockUtil::setNoBlocked(int sock, bool noblock) {
	...
	int ret = ioctl(sock, FIONBIO, &ul); //设置为非阻塞模式 //비 차단 모드로 설정
	if (ret == -1) {
		TraceL << "设置非阻塞失败！";//비 차단 설정에 실패했습니다!
	}
...
int SockUtil::setRecvBuf(int sock, int size) {
	...
	if (ret == -1) {
		TraceL << "设置接收缓冲区失败！";//수신 버퍼를 설정하지 못했습니다!
	}
	return ret;
}
int SockUtil::setSendBuf(int sock, int size) {
	...
	if (ret == -1) {
		TraceL << "设置发送缓冲区失败！";//전송 버퍼를 설정하지 못했습니다!
	}
	return ret;
}
...
int SockUtil::connect(const char *host, uint16_t port) {
    ...
    if(ret < 0 && errno!=EINPROGRESS){
        WarnL << "连接主机失败：" << host << " " << port << " " << strerror(errno);//호스트에 연결하지 못했습니다.
        close(sockfd);
        return -1;
    }
    return sockfd;
...
int SockUtil::listen(const uint16_t port, const char* localIp, int backLog) {
	...
	if ((sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1) {
		WarnL << "创建套接字失败:" << strerror(errno);//소켓 생성 실패 :
		return -1;
	}
    ...
    if (::bind(sockfd, (struct sockaddr *) &servaddr, sizeof(servaddr)) == -1) {
		WarnL << "绑定套接字失败:" << strerror(errno);//소켓 바인딩 실패 :
		close(sockfd);
		return -1;
	}
	//开始监听 //듣기 시작
	if (::listen(sockfd, backLog) == -1) {
		WarnL << "开始监听失败:" << strerror(errno);//듣기 시작 실패 :
		close(sockfd);
		return -1;
	}
...
string SockUtil::get_local_ip(int fd) {
	...
	//获取local ip and port //로컬 IP 및 포트 가져 오기
...
string SockUtil::get_local_ip() {
	...
	if (-1 == ioctl(sockfd, SIOCGIFCONF, &ifconf)) {    //获取所有接口信息 //모든 인터페이스 정보 얻기
		WarnL << "ioctl 失败:" << strerror(errno);//ioctl 실패 :
		close(sockfd);
		return "127.0.0.1";
	}
	close(sockfd);
	//接下来一个一个的获取IP地址 //다음으로 IP 주소를 하나씩 가져옵니다.
	ifreq = (struct ifreq*) buf;

	for (i = (ifconf.ifc_len / sizeof(struct ifreq)); i > 0; i--) {
		ip = inet_ntoa(((struct sockaddr_in*) &(ifreq->ifr_addr))->sin_addr);
		//排除127.0.0.1，继续下一个 //127.0.0.1을 제외하고 다음 단계를 계속합니다.
...
uint16_t SockUtil::get_local_port(int fd) {
	...
	struct sockaddr_in* addr_v4;
	socklen_t addr_len = sizeof(addr);
	//获取remote ip and port //원격 IP 및 포트 가져 오기
	if (0 == getsockname(fd, &addr, &addr_len)) {
		if (addr.sa_family == AF_INET) {
			addr_v4 = (sockaddr_in*) &addr;
...
int SockUtil::setMultiTTL(int sockFd, uint8_t ttl) {
	...
	if (ret == -1) {
		TraceL << "设置 IP_MULTICAST_TTL 失败！";//IP_MULTICAST_TTL을 설정하지 못했습니다!
	}
#endif
...
int SockUtil::setMultiIF(int sockFd, const char* strLocalIp) {
	...
	if (ret == -1) {
		TraceL << "设置 IP_MULTICAST_IF 失败！";//IP_MULTICAST_IF를 설정하지 못했습니다!
	}
#endif
	clearMulticastAllSocketOption(sockFd);
	return ret;
...
int SockUtil::setMultiLOOP(int sockFd, bool bAccept) {
	...
	if (ret == -1) {
		TraceL << "设置 IP_MULTICAST_LOOP 失败！";//IP_MULTICAST_LOOP를 설정하지 못했습니다!
	}
#endif
	clearMulticastAllSocketOption(sockFd);
	return ret;
...
int SockUtil::joinMultiAddr(int sockFd, const char* strAddr,const char* strLocalIp) {
	...
	if (ret == -1) {
		TraceL << "设置 IP_ADD_MEMBERSHIP 失败:" << strerror(errno);//IP_ADD_MEMBERSHIP 설정 실패 :
	}
#endif
	clearMulticastAllSocketOption(sockFd);
	return ret;
...
int SockUtil::leaveMultiAddr(int sockFd, const char* strAddr,const char* strLocalIp) {
	...
	if (ret == -1) {
		TraceL << "设置 IP_DROP_MEMBERSHIP 失败:" << strerror(errno);;//IP_DROP_MEMBERSHIP 설정 실패 :
	}
#endif
	clearMulticastAllSocketOption(sockFd);
	return ret;
...
int SockUtil::joinMultiAddrFilter(int sockFd, const char* strAddr,
	...
	if (ret == -1) {
		TraceL << "设置 IP_ADD_SOURCE_MEMBERSHIP 失败:" << strerror(errno);//IP_ADD_SOURCE_MEMBERSHIP 설정 실패 :
	}
#endif
	clearMulticastAllSocketOption(sockFd);
	return ret;
...
int SockUtil::leaveMultiAddrFilter(int sockFd, const char* strAddr,
	...
	ret = setsockopt(sockFd, IPPROTO_IP, IP_DROP_SOURCE_MEMBERSHIP,
			(const char*) &imr, sizeof(struct ip_mreq_source));
	if (ret == -1) {
		TraceL << "设置 IP_DROP_SOURCE_MEMBERSHIP 失败:" << strerror(errno);//IP_DROP_SOURCE_MEMBERSHIP 설정 실패 :
	}
#endif
	clearMulticastAllSocketOption(sockFd);
