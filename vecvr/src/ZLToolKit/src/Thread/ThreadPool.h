...
public:
	...
	//把任务打入线程池并异步执行 //스레드 풀에 작업을 넣고 비동기 적으로 실행
	template <typename T>
	bool async(T &&task) {
		if (!avaible) {
			return false;
	...
	//同步等待线程池执行完所有任务并退出 //스레드 풀이 모든 작업을 실행하고 종료 할 때까지 동기식으로 기다립니다.
	void wait() {
		exit();
...
private:
	...
	Priority priority;
	//发送空任务至任务列队，通知线程主动退出 //빈 작업을 작업 대기열에 보내고 스레드에게 적극적으로 종료하도록 알립니다.
	void exit() {
		avaible = false;
