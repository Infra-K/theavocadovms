...
EventPoller::EventPoller(bool enableSelfRun) {
	if (pipe(pipe_fd) == -1) {
		throw runtime_error(StrPrinter << "创建管道失败：" << errno << endl);//파이프 라인 생성 실패 :
	}
	...
	if (epoll_fd == -1) {
		close(pipe_fd[0]);
		close(pipe_fd[1]);
		throw runtime_error(StrPrinter << "创建epoll文件描述符失败：" << errno << endl);//epoll 파일 설명자를 생성하지 못했습니다.
	}
#endif //HAS_EPOLL
	initPoll();
	if (enableSelfRun) {
...
int EventPoller::addEvent(int fd, int event, PollEventCB &&cb) {
	TimeTicker();
	if (!cb) {
		WarnL << "PollEventCB 为空!";//PollEventCB가 비어 있습니다!
		return -1;
	}
#ifdef HAS_EPOLL
...
void EventPoller::async(PollAsyncCB &&asyncCb) {
	...
	std::shared_ptr<Ticker> pTicker(new Ticker(5,"唤醒主线程",FatalL,true));//메인 스레드 깨우기
...
void EventPoller::initPoll() {
	...
	if (addEvent(pipe_fd[0], Event_Read | Event_Error, [](int event) {})
			== -1) {
		FatalL << "epoll添加管道失败" << endl;//epoll이 파이프 라인을 추가하지 못했습니다.
...
void EventPoller::runLoop() {
		...
		for (int i = 0; i < nfds; ++i) {
			...
			if (fd == pipe_fd[0]) {
				...
				if (event & Event_Error) {
					WarnL << "Poller 异常退出监听。";//폴러가 비정상적으로 모니터링을 종료합니다.
					return;
				}
				if (!handlePipeEvent()) {
					InfoL << "Poller 退出监听。";//폴러가 모니터링을 종료합니다.
					return;
				}
				continue;
			}
			...
			PollEventCB eventCb;
			{
				...
				auto it = event_map.find(fd);
				if (it == event_map.end()) {
					WarnL << "未找到Poll事件回调对象!";//Poll 이벤트 콜백 개체를 찾을 수 없습니다!
	...
	while (true) {
		...
		Set_write.fdZero();
		Set_err.fdZero();
		Set_read.fdSet(pipe_fd[0]); //监听管道可读事件 //파이프 라인에서 읽을 수있는 이벤트 수신
		maxFd = pipe_fd[0];
		{
			lock_guard<mutex> lck(mtx_event_map);
			for (auto &pr : event_map) {
				...
				if (pr.second.event & Event_Write) {
					Set_write.fdSet(pr.first);//监听管道可写事件 //파이프 라인 쓰기 가능 이벤트 수신
				}
				if (pr.second.event & Event_Error) {
					Set_err.fdSet(pr.first);//监听管道错误事件 //파이프 라인 오류 이벤트 수신
				}
			}
		}
