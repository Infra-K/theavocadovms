...
CMD_help::CMD_help() 
{
	...
	(*parser) << Option('c', "cmd", Option::ArgNone, "列出所有命令", [](OutStream *stream,const char *arg) {//모든 명령 나열
		_StrPrinter printer;
...
{
	parser.reset(new OptionParser(nullptr));
	(*parser) << Option('l', "list", Option::ArgNone, "列出所有媒体列表", [](OutStream *stream,const char *arg) {//모든 미디어 나열
		_StrPrinter printer;
