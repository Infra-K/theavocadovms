...
public:
	...
	OptionParser(const OptionCompleted &_cb) {
		onCompleted = _cb;
		helper = Option('h', "help", Option::ArgNone, "获取此帮助", [this](OutStream *stream,const char *arg)->bool {//이 도움 받기
			_StrPrinter printer;
			for (auto &pr : options) {
...
public:
	...
	const char *description() const override {
		return "查看rtsp服务器相关信息.";//rtsp 서버에 대한 정보를 봅니다.
	}
};
...
public:
	...
	const char *description() const override {
		return "查看rtmp服务器相关信息.";//rtmp 서버에 대한 정보를 봅니다.
	}
};
