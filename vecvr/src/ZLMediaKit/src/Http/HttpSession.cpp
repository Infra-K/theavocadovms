...
void HttpSession::onRecv(const char *data,int size){
	...
	if (m_strRcvBuf.size() + size >= reqSize) {
		WarnL << "接收缓冲区溢出:" << m_strRcvBuf.size() + size << "," << reqSize;//수신 버퍼 오버 플로우 :
		shutdown();
		return;
	}
	...
	while ((index = m_strRcvBuf.find("\r\n\r\n")) != std::string::npos) {
		...
		case Http_success:
			...
			break;
		case Http_moreData:
			//需要更多数据,恢复数据并退出 //더 많은 데이터 필요, 데이터 복원 및 종료
...
void HttpSession::onManager() {
	...
	if(m_ticker.elapsedTime() > keepAliveSec * 1000) {
		//1分钟超时 //1 분 제한 시간
		WarnL<<"HttpSession超时断开！";//"HttpSession 시간 초과 연결이 끊어졌습니다!"
		shutdown();
	}
}
...
*/
	...
	if (strPath != "/") {
		...
		strRet += "/";
		strRet += "\">";
		strRet += "根目录";//strEet + = "루트 디렉토리"; strEet + = "상위 디렉토리";
		strRet += "</a></li>\r\n";
...
inline HttpSession::HttpCode HttpSession::Handle_Req_POST() {
	...
	if ((int) m_strRcvBuf.size() < iContentLen) {
		return Http_moreData; //需要更多数据 //더 많은 데이터 필요
	}
