...
RtpParser::RtpParser(const string& sdp) {
	...
	if (!m_bHaveVideo && !m_bHaveAudio) {
		throw std::runtime_error("不支持该RTSP媒体格式");//RTSP 미디어 형식은 지원되지 않습니다.
	}
...
inline bool RtpParser::inputVideo(const RtpPacket& rtppack,
	...
	if (nal.type == 28) {
		...
		if (rtppack.sequence != (uint16_t)(m_h264frame.sequence + 1)) {
			m_h264frame.data.clear();
			WarnL << "丢包,帧废弃：" << rtppack.sequence << "," << m_h264frame.sequence;//패킷 손실, 프레임 폐기 :
			return false;
		}
		m_h264frame.sequence = rtppack.sequence;
		if (fu.E == 1) {
	...
	WarnL << nal.type;
	return false;
	// 29 FU-B     单NAL单元B模式 단일 NAL 장치 B 모드
	// 24 STAP-A   单一时间的组合包 단일 시간 패키지
	// 25 STAP-B   单一时间的组合包 단일 시간 패키지
	// 26 MTAP16   多个时间的组合包 여러 시간 패키지
	// 27 MTAP24   多个时间的组合包
	// 0 udef
	// 30 udef
	// 31 udef
...
inline void RtpParser::onGetAudioTrack(const RtspTrack& audio) {
	...
	if (audio.trackSdp.find("mpeg4-generic") == string::npos) {
		throw std::runtime_error("只支持aac格式的音频！");//aac 형식의 오디오 만 지원합니다!
	}
	...
	if (fConfigStr.size() != 4) {
		throw std::runtime_error("解析aac格式头失败！");//aac 형식 헤더를 구문 분석하지 못했습니다!
	}
	m_strAudioCfg.clear();
	unsigned int cfg1;
...
inline void RtpParser::onGetVideoTrack(const RtspTrack& video) {
	if (video.trackSdp.find("H264") == string::npos) {
		throw std::runtime_error("只支持264格式的视频！");//264 형식 비디오 만 지원합니다!
	}
	...
	if (!getAVCInfo(strTmp, m_iVideoWidth, m_iVideoHeight, m_fVideoFps)) {
		throw std::runtime_error("解析SPS失败！");//SPS를 구문 분석하지 못했습니다!
	}
}
