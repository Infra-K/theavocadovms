...
public:
	...
	const RingType::Ptr &getRing() const {
		//获取媒体源的rtp环形缓冲 //미디어 소스의 rtp 링 버퍼 가져 오기
		return m_pRing;
	}

	const string& getSdp() const {
		//获取该源的媒体描述信息 //소스의 미디어 설명 정보 가져 오기
		return m_strSdp;
	}

	virtual void regist() {
		//注册该源，注册后rtsp服务器才能找到该源 //소스를 등록하면 rtsp 서버가 등록 후 소스를 찾을 수 있습니다.
	...
	virtual void onGetSDP(const string& sdp) {
		//派生类设置该媒体源媒体描述信息 //파생 클래스는 미디어 소스의 미디어 설명 정보를 설정합니다.
		this->m_strSdp = sdp;
	}
...
private:
	string m_strSdp; //媒体描述信息 //미디어 설명
	string m_strApp; //媒体app //미디어 앱
	string m_strId; //媒体id //미디어 ID
	RingType::Ptr m_pRing; //rtp环形缓冲 //rtp 링 버퍼
	ThreadPool &m_thPool;
	static unordered_map<string, unordered_map<string, weak_ptr<RtspMediaSource> > > g_mapMediaSrc; //静态的媒体源表 //정적 미디어 소스 테이블
	static recursive_mutex g_mtxMediaSrc; ///访问静态的媒体源表的互斥锁 //정적 미디어 소스 테이블에 대한 액세스를위한 뮤텍스 잠금
};
