...
//播放，指定是否走rtp over tcp //재생, tcp를 통해 rtp로 이동할지 여부 지정
...
void RtspPlayer::onConnect(const SockException &err)
{
	...
	//发送DESCRIBE命令后处理函数：HandleResDESCRIBE //DESCRIBE 명령 전송 후 처리 기능 : HandleResDESCRIBE
...
{
	//发送DESCRIBE命令后的回复 //DESCRIBE 명령을 보낸 후 응답
	if (parser.Url() != "200") {
		throw std::runtime_error(StrPrinter << "DESCRIBE:" << parser.Url() << " " << parser.Tail() << endl);
	}
	...
	if (!m_uiTrackCnt) {
		throw std::runtime_error("解析SDP失败");//SDP를 구문 분석하지 못했습니다.
	}
...
void RtspPlayer::HandleResSETUP(const Parser& parser, unsigned int uiTrackIndex) 
{
	...
	if (uiTrackIndex < m_uiTrackCnt - 1) {
		//需要继续发送SETUP命令 //SETUP 명령을 계속 전송해야합니다.
	...
	for (unsigned int i = 0; i < m_uiTrackCnt && m_eType != RTP_TCP; i++) {
		...
		pUdpSockRef->setOnRead([srcIP,i,weakSelf](const Socket::Buffer::Ptr &buf, struct sockaddr *addr) {
			...
			if(((struct sockaddr_in *)addr)->sin_addr.s_addr != srcIP) {
				...
				WarnL << "收到请他地址的UDP数据:" << strAddr; //그의 주소에서 UDP 데이터 수신 :
				return;
			}
...
//注意：当字符串为空时，也会返回一个空字符串 //참고 : 문자열이 비어 있으면 빈 문자열도 반환됩니다.
...
inline void RtspPlayer::splitRtp(unsigned char* pucRtp, unsigned int uiLen) 
{
	...
	while (uiLen >= 4) {
		if (rtp_ptr[0] == '$') {
			...
			if (length > 1600) {
				//没有大于MTU的包 //MTU보다 큰 패킷 없음
				//WarnL << "没有大于MTU的包:" << length;//MTU보다 큰 패킷이 없습니다.
				rtp_ptr += 1;
				uiLen -= 1;
				continue;
			}
			if ((unsigned int) length + 4 + 4 > uiLen) {
				//buf 太小,还没到该RTP包的结尾 //buf가 너무 작아 RTP 패킷의 끝에 도달하지 않았습니다.
				break;
			}
			auto nextPkt = rtp_ptr + length + 4;
			if (*nextPkt != '$' && memcmp(nextPkt,"RTSP",4)!=0 ) {
				//没有找到该包的尾部 //패키지의 끝을 찾을 수 없습니다.
				//WarnL << "没有找到该包的尾部"; //패키지의 끝을 찾을 수 없습니다.
				rtp_ptr += 1;
				uiLen -= 1;
				continue;
			}
		...
		if (pos == NULL) {
			//缓存里面没有任何RTP包 //캐시에 RTP 패킷이 없습니다.
			//WarnL << "缓存里面没有任何RTP包"; //캐시에 RTP 패킷이 없습니다.
			uiLen = 0;
			break;
		}
		//有RTP包起始头 //RTP 패킷 시작 헤더 포함
		uiLen -= (pos - rtp_ptr);
		rtp_ptr = pos;
	}
	m_uiRtpBufLen = uiLen;
...
inline bool RtspPlayer::HandleOneRtp(int iTrackidx, unsigned char *pucData, unsigned int uiLen) 
{
	...
	//序列号 //일련 번호
	memcpy(&rtppt.sequence,pucData+2,2);//内存对齐 //메모리 정렬
	...
	if (m_abSortStarted[iTrackidx]) {
		...
		if (m_aui64SeqOkCnt[iTrackidx] >= clearCount) {
			//网络环境改善，需要清空排序缓存 //네트워크 환경이 개선되고 정렬 캐시를 지워야합니다.
			m_aui64SeqOkCnt[iTrackidx] = 0;
