...
recursive_mutex RtspSession::g_mtxGetter; //对quicktime上锁保护 //빠른 시간을위한 잠금 보호
...
void RtspSession::onError(const SockException& err) 
{
	...
	if (m_bBase64need && err.getErrCode() == Err_eof) {
		//quickTime http postter,正在发送rtp; QuickTime只是断开了请求连接,请继续发送rtp //quickTime http postter, rtp 전송; QuickTime이 방금 요청 연결을 끊었습니다. rtp 전송을 계속하십시오.
		sock = nullptr;
		lock_guard<recursive_mutex> lock(g_mtxPostter);
		//为了保证脱离TCPServer后还能正常运作,需要保持本对象的强引用 //TCPServer를 떠난 후 정상적인 작동을 보장하려면이 개체에 대한 강력한 참조를 유지해야합니다.
		g_mapPostter.emplace(this, dynamic_pointer_cast<RtspSession>(shared_from_this()));
		TraceL << "quickTime已经不再发送请求"; //quickTime은 더 이상 요청을 보내지 않습니다.
	}
}
...
void RtspSession::onRecv(const Socket::Buffer::Ptr &pBuf) 
{
	...
	m_pcBuf = tmp;
	if (m_bBase64need) {
		//quicktime 加密后的rtsp请求，需要解密 //Quicktime으로 암호화 된 RTSP 요청을 해독해야합니다.
		av_base64_decode((uint8_t *) m_pcBuf, pBuf->data(), sizeof(tmp));
		m_parser.Parse(m_pcBuf); //rtsp请求解析 //rtsp 요청 구문 분석
		//TraceL << buf;
	} 
	else {
		//TraceL << request;
	...
	string strCmd = m_parser.Method(); //提取出请求命令字 //요청 명령어 추출
...
bool RtspSession::handleReq_Describe() {
	...
	if (m_strSession.size() != 0) {
		//会话id这时还没生成，这个逻辑可以注释以提高响应速度 //세션 ID가 아직 생성되지 않았습니다.이 로직에 주석을 달아 응답 속도를 높일 수 있습니다.
		return false;
	}
	if (!findStream()) {
		//未找到相应的MediaSource //해당 MediaSource를 찾을 수 없습니다.
		send_StreamNotFound();
		return false;
	}
//回复sdp //sdp에 답장
...
//处理setup命令，该函数可能进入多次 //설정 명령을 처리하는 중, 기능이 여러 번 입력 될 수 있습니다.
//track id
	...
	if (trackIdx == -1) {
		//未找到相应track //해당 트랙을 찾을 수 없습니다.
		return false;
	}
	RtspTrack &trackRef = m_aTrackInfo[trackIdx];
	if (trackRef.inited) {
		//已经初始化过该Track //트랙이 초기화되었습니다.
		return false;
	}
	trackRef.inited = true; //现在初始化 //지금 초기화
	...
	case PlayerBase::RTP_UDP: {
		...
		if (!pSock) {
			//分配端口失败 //포트를 할당하지 못했습니다.
			WarnL << "分配端口失败"; //포트를 할당하지 못했습니다.
			send_NotAcceptable();
			return false;
		}
...
bool RtspSession::handleReq_Teardown() 
{
	...
	send(m_pcBuf, n);
	TraceL << "播放器断开连接!";//"플레이어 연결이 끊어졌습니다!"
...
//Poster 找到 Getter //포스터 찾기 Getter
	auto it = g_mapGetter.find(sessioncookie);
	if (it == g_mapGetter.end()) {
		return false;
	}
	m_bBase64need = true;
//Poster 找到Getter的SOCK //포스터 찾기 Getter 's SOCK
