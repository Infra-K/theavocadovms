...
void RtpMaker_H264::makeRtp(const char* pcData, int iLen, uint32_t uiStamp) {
	...
	if (iLen > iSize) { //超过MTU //MTU를 통해
		const unsigned char s_e_r_Start = 0x80;
		const unsigned char s_e_r_Mid = 0x00;
		const unsigned char s_e_r_End = 0x40;
		//获取帧头数据，1byte //프레임 헤더 데이터 가져 오기, 1 바이트
		unsigned char naluType = *((unsigned char *) pcData) & 0x1f; //获取NALU的5bit 帧类型 //NALU의 5 비트 프레임 유형 가져 오기

		unsigned char nal_ref_idc = *((unsigned char *) pcData) & 0x60; //获取NALU的2bit 帧重要程度 00 可以丢 11不能丢 //NALU 00의 2 비트 프레임 중요성을 얻을 수 있습니다. 11 손실 될 수 없습니다.
		//nal_ref_idc = 0x60;
		//组装FU-A帧头数据 2byte //FU-A 프레임 헤더 데이터 2 바이트 조립
		unsigned char f_nri_type = nal_ref_idc + 28;//F为0 1bit,nri上面获取到2bit,28为FU-A分片类型5bit //F는 0 1 비트, 2 비트는 nri, 28은 FU-A 조각화 유형의 5 비트입니다.
		unsigned char s_e_r_type = naluType;
		bool bFirst = true;
		bool mark = false;
		int nOffset = 1;
		while (!mark) {
			if (iLen < nOffset + iSize) {			//是否拆分结束 //분할 종료 여부
				iSize = iLen - nOffset;
				mark = true;
