...
namespace media {
    ...
    // The canonical example use-case is holding the list of ranges of buffered bytes or times in a <video> tag.
    ...
    size_t Ranges<T>::Add(T start, T end) {
        ...
        //DCheckLT(start, end);
        size_t i;
        // Walk along the array of ranges until |start| is no longer larger than the current interval's end.
        ...
        // Easy cases done.  Getting here means there is overlap between [start,end) and the existing ranges.        
