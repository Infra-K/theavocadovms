...
namespace media {
    ...
    // Check if a slice includes memory management control operation 5,
    // which results in some |pic_order_cnt| state being cleared.
    ...
    bool H264POC::ComputePicOrderCnt(const H264SPS* sps,
        ...
        // Check for invalid (including duplicate) |frame_num| values.
        // All cases are treated as gaps, which is to say that nothing is done. (Gaps don't affect POC computation.)
        ...
        // Based on T-REC-H.264 8.2.1, "Decoding process for picture order count",
