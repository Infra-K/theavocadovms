...
namespace media {
    ...
    bool H264BitReader::HasMoreRBSPData() {
        ...
        // If there is no more RBSP data, then |curr_byte_| contains the stop bit and zero padding.
        // Check to see if there is other data instead.
        // (We don't actually check for the stop bit itself, instead treating the invalid case of all trailing zeros identically).
        ...
        // not be equal to 0x00"), some streams have trailing null bytes anyway.
        // We don't handle emulation prevention sequences because HasMoreRBSPData() is
