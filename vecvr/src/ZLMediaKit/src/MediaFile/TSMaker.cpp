...
void TSMaker::flush() {
	...
	if ((m_uiWritePacketNum % 40) == 0)             //每40个包打一个 pat,一个pmt //가방 40 개당 팻 1 개와 pmt 1 개
			{
		CreatePAT();								//创建PAT //PAT 생성
		CreatePMT();								//创建PMT
	}
	...
	//PID = TS_H264_PID, 페이로드 단위의 시작 표시기가 아님 _play_init = 0x00, ada_field_C, 0x03, 조정 필드 및 페이로드 포함;
	...
	m_pVideo_pes->ESlen = 0;
	fwrite(acTSbuf, 188, 1, m_pOutVideoTs);   //将一包数据写入文件 //파일에 데이터 패킷 쓰기
	m_uiWritePacketNum++;
...
void TSMaker::CreatePAT() {
	...
	// PID = 0x00, 페이로드 단위 시작 표시기 _play_init = 0x01, ada_field_C, 0x01, 페이로드 만;
	TsHeader2buffer(&ts_header, aucPat);
	pucPat[4] = 0;                                //自适应段的长度为0 //가변 세그먼트의 길이는 0입니다.
	pucPat += 5;
	ts_pat.table_id = 0x00;
	ts_pat.section_syntax_indicator = 0x01;
	ts_pat.zero = 0x00;
	ts_pat.reserved_1 = 0x03;                     //设置为11； //11로 설정합니다.
	ts_pat.section_length = 0x0d;                 //pat结构体长度 16个字节减去上面的3个字节 //pat 구조의 길이는 16 바이트에서 위의 3 바이트를 뺀 값입니다.
	ts_pat.transport_stream_id = 0x01;
	ts_pat.reserved_2 = 0x03;                     //设置为11；
	ts_pat.version_number = 0x00;
	ts_pat.current_next_indicator = 0x01;         //当前的pat 有效 //현재 팻이 유효합니다
	ts_pat.section_number = 0x00;
	ts_pat.last_section_number = 0x00;
	ts_pat.program_number = 0x01;
	ts_pat.reserved_3 = 0x07;                     //设置为111； //111로 설정;
	ts_pat.program_map_PID = TS_PMT_PID;          //PMT的PID //PMT의 PID
	ts_pat.CRC_32 = ui32PAT_CRC;                  //传输过程中检测的一种算法值 先设定一个填充值 //전송 중 감지 된 알고리즘 값, 먼저 채우기 값 설정
	...
	fwrite(aucPat, 188, 1, m_pOutVideoTs); //将PAT包写入文件 //파일에 PAT 패키지 쓰기
	return;
}
...
void TSMaker::CreatePMT() {
	...
	memset(aucPmt, 0xFF, TS_PACKET_SIZE);                           //将一个包填成0xFF //0xFF로 패킷 채우기
	pucPmt = aucPmt;

	CreateTsHeader(&ts_header, TS_PMT_PID, 0x01, 0x01); //PID = 0x00,有效荷载单元起始指示符_play_init = 0x01, ada_field_C,0x01,仅有有效负载；//PID = 0x00, 유효 부하 단위 시작 표시기 _play_init = 0x01, ada_field_C, 0x01, 유효 부하 만;
	TsHeader2buffer(&ts_header, aucPmt);
	pucPmt[4] = 0;                                              //自适应段的长度为0 //가변 세그먼트의 길이는 0입니다.
	pucPmt += 5;
	ts_pmt.table_id = 0x02;
	ts_pmt.section_syntax_indicator = 0x01;
	ts_pmt.zero = 0x00;
	ts_pmt.reserved_1 = 0x03;
	ts_pmt.section_length = 0x17;             //PMT结构体长度 16 + 5 + 5个字节减去上面的3个字节 //PMT 구조의 길이는 16 + 5 + 5 바이트에서 위의 3 바이트를 뺀 값입니다.
	ts_pmt.program_number = 01;                                        //只有一个节目 //단 하나의 쇼
	ts_pmt.reserved_2 = 0x03;
	ts_pmt.version_number = 0x00;
	ts_pmt.current_next_indicator = 0x01;                            //当前的PMT有效 //현재 PMT가 유효합니다.
	ts_pmt.section_number = 0x00;
	ts_pmt.last_section_number = 0x00;
	ts_pmt.reserved_3 = 0x07;
	ts_pmt.PCR_PID = TS_H264_PID;                                       //视频PID //비디오 PID
	ts_pmt.reserved_4 = 0x0F;
	ts_pmt.program_info_length = 0x00;                             //后面无 节目信息描述 //뒤에 프로그램 정보 설명 없음
	ts_pmt.stream_type_video = PMT_STREAM_TYPE_VIDEO;                   //视频的类型 //비디오 유형
	ts_pmt.reserved_5_video = 0x07;
	ts_pmt.elementary_PID_video = TS_H264_PID;                         //视频的PID //비디오의 PID
	ts_pmt.reserved_6_video = 0x0F;
	ts_pmt.ES_info_length_video = 0x00;                            //视频无跟随的相关信息 //동영상이 관련 정보를 따르지 않습니다.
	ts_pmt.stream_type_audio = PMT_STREAM_TYPE_AUDIO;                    //音频类型 //오디오 유형
	ts_pmt.reserved_5_audio = 0x07;
	ts_pmt.elementary_PID_audio = TS_AAC_PID;                           //音频PID //오디오 PID
	ts_pmt.reserved_6_audio = 0x0F;
	ts_pmt.ES_info_length_audio = 0x00;                            //音频无跟随的相关信息 //오디오 비추 종 관련 정보
	...
	pucPmt[12] = ts_pmt.stream_type_video;               //视频流的stream_type //비디오 스트림의 Stream_type
	...
	pucPmt[17] = ts_pmt.stream_type_audio;               //音频流的stream_type //오디오 스트림의 Stream_type
	...
	*(aucPmt + 7) = iLen;
	...
	fwrite(aucPmt, 188, 1, m_pOutVideoTs);                             //将PAT包写入文件 //파일에 PAT 패키지 쓰기
}
...
void TSMaker::WriteAdaptive_flags_Head( Ts_Adaptation_field * pTsAdaptationField, uint64_t ui64VideoPts) {
	...
	pTsAdaptationField->PCR_flag = 1;                                   //只用到这个 //이것을 사용하십시오
	...
	pTsAdaptationField->adaptation_field_length = 7;                     //占用7位 //7 비트 점유
...
int TSMaker::inputH264(const char* pcData, uint32_t ui32Len, uint64_t ui64Time) {
	...
	WriteAdaptive_flags_Head(&ts_adaptation_field_Head, ui64Time); //填写自适应段标志帧头 //적응 형 세그먼트 플래그의 프레임 헤더를 입력합니다.
	m_pVideo_pes->packet_start_code_prefix = 0x000001;
	m_pVideo_pes->stream_id = TS_H264_STREAM_ID; //E0~EF表示是视频的,C0~DF是音频,H264-- E0 //E0 ~ EF는 비디오, C0 ~ DF는 오디오, H264-- E0
	m_pVideo_pes->marker_bit = 0x02;
	m_pVideo_pes->PES_scrambling_control = 0x00;                   //人选字段 存在，不加扰 //후보 필드가 스크램블없이 존재합니다.
	...
	m_pVideo_pes->PES_header_data_length = 0x0A; //后面的数据包括了PTS和 DTS所占的字节数 //다음 데이터에는 PTS 및 DTS가 차지하는 바이트 수가 포함됩니다.
...
int TSMaker::inputAAC(const char* pcData, uint32_t ui32Len, uint64_t ui64Pts) {
	...
	Ts_Adaptation_field ts_adaptation_field_Head;
	WriteAdaptive_flags_Tail(&ts_adaptation_field_Head); //填写自适应段标志帧头 //적응 형 세그먼트 플래그의 프레임 헤더를 입력합니다.
	m_pAudio_pes->packet_start_code_prefix = 0x000001;
	m_pAudio_pes->stream_id = TS_AAC_STREAM_ID; //E0~EF表示是视频的,C0~DF是音频,H264-- E0 //E0 ~ EF는 비디오, C0 ~ DF는 오디오, H264-- E0
	m_pAudio_pes->marker_bit = 0x02;
	m_pAudio_pes->PES_scrambling_control = 0x00;                   //人选字段 存在，不加扰 //후보 필드가 스크램블없이 존재합니다.
	...
	m_pAudio_pes->PES_header_data_length = 0x0A; //后面的数据包括了PTS //다음 데이터에는 PTS가 포함됩니다.
...
void TSMaker::WriteAdaptive_flags_Tail(Ts_Adaptation_field* pTsAdaptationField) {
	...
	pTsAdaptationField->random_access_indicator = 0;
	pTsAdaptationField->elementary_stream_priority_indicator = 0;
	pTsAdaptationField->PCR_flag = 0;                                   //只用到这个 //이것을 사용하십시오
	pTsAdaptationField->OPCR_flag = 0;
	pTsAdaptationField->splicing_point_flag = 0;
	pTsAdaptationField->transport_private_data_flag = 0;
	pTsAdaptationField->adaptation_field_extension_flag = 0;
	...
	pTsAdaptationField->adaptation_field_length = 1;               //占用1位标志所用的位 //플래그가 사용하는 1 비트를 차지합니다.
...
void TSMaker::CreateAdaptive_Ts(Ts_Adaptation_field * pTsAdaptationField, unsigned char * pucTs, unsigned int uiAdaptiveLength) {
	unsigned int uiCurrentAdaptiveLength = 1;                       //当前已经用的自适应段长度 //현재 사용되는 가변 세그먼트 길이
	unsigned char ucAdaptiveflags = 0;                                   //自适应段的标志 //적응 세그먼트의 부호
	unsigned char *pucTmp = pucTs;
	//填写自适应字段 //적응 형 필드 채우기
	if (pTsAdaptationField->adaptation_field_length > 0) {
		pucTs += 1;                                     //自适应段的一些标志所占用的1个字节 //적응 형 세그먼트의 일부 기호가 차지하는 1 바이트
		...
		if (pTsAdaptationField->private_data_len > 0) {
			...
			if (1 + pTsAdaptationField->private_data_len
					> static_cast<unsigned char>(uiAdaptiveLength - uiCurrentAdaptiveLength)) {
				...
				pucTs += 1;
				memcpy(pucTs, pTsAdaptationField->private_data, pTsAdaptationField->private_data_len);
				pucTs += pTsAdaptationField->private_data_len;

				uiCurrentAdaptiveLength += (1 + pTsAdaptationField->private_data_len);
			}
		}
		...
		*pucTmp = ucAdaptiveflags;    //将标志放入内存 //메모리에 플래그 넣기
	}
...
void TSMaker::PES2TS(TsPes * pTsPes, unsigned int uiPID, Ts_Adaptation_field * pTsAdaptationFieldHead, uint64_t ui64Dts) {
	TsPacketHeader ts_header;
	unsigned int uiAdaptiveLength = 0;                                //要填写0XFF的长度 //0XFF의 길이를 채우려면
	unsigned int uiFirstPacketLoadLength = 188 - 4 - 1 - pTsAdaptationFieldHead->adaptation_field_length - 19; //分片包的第一个包的负载长度 //단편화 된 패킷의 첫 번째 패킷의 페이로드 길이
	const char * pcNeafBuf = pTsPes->ES;                              //分片包 总负载的指针 //조각난 패킷의 총로드에 대한 포인터
	...
	while (true) {
		...
		if (bFirstPkt) {
			...
			pucTSBuf = aucTSbuf;
			CreateTsHeader(&ts_header, uiPID, 0x01, 0x03); //PID = TS_H264_PID,有效荷载单元起始指示符_play_init = 0x01, ada_field_C,0x03,含有调整字段和有效负载 ；//PID = TS_H264_PID, 페이로드 단위 시작 표시기 _play_init = 0x01, ada_field_C, 0x03, 조정 필드 및 페이로드 포함;
			//PID = TS_H264_PID, 페이로드 단위 시작 표시기 _play_init = 0x01, ada_field_C, 0x03, 조정 필드 및 페이로드 포함;
			TsHeader2buffer(&ts_header, aucTSbuf);
			pucTSBuf += 4; //写入TS 头 //TS 헤더 쓰기
			if (pTsPes->ESlen > uiFirstPacketLoadLength) {
				//计算分片包的第一个包的负载长度 //조각난 패킷의 첫 번째 패킷의 페이로드 길이 계산
				uiAdaptiveLength = 188 - 4 - 1 - ((pTsPes->ESlen - uiFirstPacketLoadLength) % 184); //要填写0XFF的长度,最后一个包有自适应 //0XFF의 길이를 채우기 위해 마지막 패킷은
				pucTSBuf[0] = pTsAdaptationFieldHead->adaptation_field_length; //自适应字段的长度，自己填写的 //적응 필드의 길이, 직접 입력
				pucTSBuf += 1;
				CreateAdaptive_Ts(pTsAdaptationFieldHead, pucTSBuf, (uiAdaptiveLength)); //填写自适应字段 //적응 형 필드 채우기
				pucTSBuf += pTsAdaptationFieldHead->adaptation_field_length; //填写自适应段所需要的长度 //적응 형 세그먼트의 필수 길이를 입력합니다.
			} 
			else {
				uiAdaptiveLength = uiFirstPacketLoadLength - pTsPes->ESlen;
				pucTSBuf[0] = pTsAdaptationFieldHead->adaptation_field_length + uiAdaptiveLength; //自适应字段的长度，自己填写的 //적응 필드의 길이, 직접 입력
				pucTSBuf += 1;
				CreateAdaptive_Ts(pTsAdaptationFieldHead, pucTSBuf, uiAdaptiveLength); //填写自适应字段 //적응 형 필드 채우기
			...
			//将包写入文件 //파일에 패키지 쓰기
			fwrite(aucTSbuf, 188, 1, m_pOutVideoTs);                      //将一包数据写入文件 //파일에 데이터 패킷 쓰기
			continue;
		}
		if (pTsPes->ESlen >= 184) {
			//处理中间包 //중간 패키지 취급
			//memset(TSbuf,0,TS_PACKET_SIZE);
			pucTSBuf = aucTSbuf;
			CreateTsHeader(&ts_header, uiPID, 0x00, 0x01); //PID = TS_H264_PID,不是有效荷载单元起始指示符_play_init = 0x00, ada_field_C,0x01,仅有有效负载； //PID = TS_H264_PID, 페이로드 단위의 시작 표시기가 아님 _play_init = 0x00, ada_field_C, 0x01, 페이로드 만;
			TsHeader2buffer(&ts_header, aucTSbuf);
			pucTSBuf += 4;
		...
		if (pTsPes->ESlen == 183) {
			//memset(TSbuf,0,TS_PACKET_SIZE);
			pucTSBuf = aucTSbuf;
			CreateTsHeader(&ts_header, uiPID, 0x00, 0x03); //PID = TS_H264_PID,不是有效荷载单元起始指示符_play_init = 0x00, ada_field_C,0x03,含有调整字段和有效负载； //PID = TS_H264_PID, 페이로드 단위의 시작 표시기가 아님 _play_init = 0x00, ada_field_C, 0x03, 조정 필드 및 페이로드 포함;
			TsHeader2buffer(&ts_header, aucTSbuf);
			pucTSBuf += 4;
			pucTSBuf[0] = 1;
			pucTSBuf[1] = 0x00;
		...
		memcpy(pucTSBuf, pcNeafBuf, pTsPes->ESlen); //183就丢弃一字节 //183 화
		pTsPes->ESlen = 0;
		fwrite(aucTSbuf, 188, 1, m_pOutVideoTs);   //将一包数据写入文件 //파일에 데이터 패킷 쓰기
		break;
	}
