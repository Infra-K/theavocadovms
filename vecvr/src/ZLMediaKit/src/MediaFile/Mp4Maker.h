...
{
public:
	time_t ui64StartedTime; //GMT标准时间，单位秒 // GMT 표준 시간 (초)
	time_t ui64TimeLen;//录像长度，单位秒 // 비디오 길이, 단위 초
	off_t ui64FileSize;//文件大小，单位BYTE // 파일 크기, 단위 BYTE
	string strFilePath;//文件路径 // 파일 경로
	string strFileName;//文件名称 // 파일 이름
	string strFolder;//文件夹路径 // 폴더 경로
	string strUrl;//播放路径 // 재생 경로
	string strAppName;//应用名称 // 응용 프로그램 이름
	string strStreamId;//流ID // 스트림 ID
};
class Mp4Maker {
public:
	...
	virtual ~Mp4Maker();
	//时间戳：参考频率1000 //타임 스탬프 : 참조 주파수 1000
