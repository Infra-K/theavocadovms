...
void RtmpPlayer::play(const char* strUrl, const char * , const char *, eRtpType) {
    ...
    if (!m_strApp.size() || !m_strStream.size()) {
        _onPlayResult(SockException(Err_other,"rtmp url非法")); //rtmp URL이 잘못되었습니다.
        return;
    }
...
inline void RtmpPlayer::send_connect() {
	...
	//未使用代理 //사용 된 프록시 없음
	obj.set("fpad", false);
	//参考librtmp,什么作用? //librtmp를 참조하면 어떤 역할을합니까?
	obj.set("capabilities", 15);
	//SUPPORT_VID_CLIENT_SEEK 支持seek SUPPORT_VID_CLIENT_SEEK 지원 검색
...
inline void RtmpPlayer::send_play() {
	...
	auto fun = [this](AMFValue &val) {
		...
		auto code = val["code"].as_string();
		if(level != "status"){
			throw std::runtime_error(StrPrinter <<"play 失败：" << level << " " << code << endl);//플레이 실패 :
		}
	};
	addOnStatusCB(fun);
	addOnStatusCB(fun);
...
inline void RtmpPlayer::send_pause(bool bPause) {
	...
	auto fun = [this,bPause](AMFValue &val){
        ...
        if(level != "status") {
            if(!bPause){
                throw std::runtime_error(StrPrinter <<"pause 恢复播放失败：" << level << " " << code << endl);//일시 중지 후 재생을 다시 시작하지 못했습니다.
            }
        }
		else {
            m_bPaused = bPause;
