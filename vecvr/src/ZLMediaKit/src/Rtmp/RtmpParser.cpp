...
RtmpParser::RtmpParser(const AMFValue &val) {
	...
	if (videoCodec.type() == AMF_STRING) {
        if (videoCodec.as_string() == "avc1") {
            ...
            InfoL << "不支持RTMP视频格式：" << videoCodec.as_string();//RTMP 비디오 형식을 지원하지 않습니다.
        }
    }
        ...
        if (videoCodec.as_integer() == 7) {
            ...
            InfoL << "不支持RTMP视频格式：" << videoCodec.as_integer();//RTMP 비디오 형식을 지원하지 않습니다.
        }
    }
	...
	if (audioCodec.type() == AMF_STRING) {
		if (audioCodec.as_string() == "mp4a") {
			...
			InfoL << "不支持RTMP音频格式：" << audioCodec.as_string();//RTMP 오디오 형식을 지원하지 않습니다.
		}
    }
        ...
        if (audioCodec.as_integer() == 10) {
            ...
            InfoL << "不支持RTMP音频格式：" << audioCodec.as_integer();//RTMP 오디오 형식을 지원하지 않습니다.
        }
    }    
    
    if (!m_bHaveVideo && !m_bHaveAudio) {
        throw std::runtime_error("不支持该RTMP媒体格式");//RTMP 미디어 형식은 지원되지 않습니다.
    }
...
inline void RtmpParser::onGetH264(const char* pcData, int iLen, uint32_t ui32TimeStamp) {
	...
	m_h264frame.data.assign("\x0\x0\x0\x1", 4);  //添加264头 //264 헤더 추가
