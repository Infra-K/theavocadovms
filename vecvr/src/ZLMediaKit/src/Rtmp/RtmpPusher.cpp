...
RtmpPusher::RtmpPusher(const char *strApp,const char *strStream) {
    ...
    if (!src) {
        auto strErr = StrPrinter << "媒体源：" << strApp << "/" << strStream << "不存在" << endl;//미디어 소스 : //존재하지 않는다
...
void RtmpPusher::publish(const char* strUrl) {
    ...
    if (!m_strApp.size() || !m_strStream.size()) {
        onPublishResult(SockException(Err_other,"rtmp url非法"));//rtmp URL이 잘못되었습니다.
        return;
    }
...
inline void RtmpPusher::send_connect() {
	...
	addOnResultCB([this](AMFDecoder &dec) {
		...
		if(level != "status") {
			throw std::runtime_error(StrPrinter <<"connect 失败：" << level << " " << code << endl);//연결 실패:
		}
		send_createStream();
	});
}
...
inline void RtmpPusher::send_publish() {
	...
	addOnStatusCB([this](AMFValue &val) {
		...
		auto code = val["code"].as_string();
		if(level != "status") {
			throw std::runtime_error(StrPrinter <<"publish 失败：" << level << " " << code << endl);//게시 실패 :
		}
		//start send media
		send_metaData();
	});
...
inline void RtmpPusher::send_metaData(){
    ...
    if (!src) {
        throw std::runtime_error("媒体源已被释放");//미디어 소스가 공개되었습니다
    }
    if (!src->ready()) {
        throw std::runtime_error("媒体源尚未准备就绪");//미디어 소스가 아직 준비되지 않았습니다.
    }
    ...
    m_pRtmpReader->setDetachCB([weakSelf]() {
        auto strongSelf = weakSelf.lock();
        if(strongSelf) {
            strongSelf->onShutdown(SockException(Err_other,"媒体源被释放"));//미디어 소스 공개
            strongSelf->teardown();
        }
    });
...
void RtmpPusher::onCmd_onStatus(AMFDecoder &dec) {
	...
	if(val.type() != AMF_OBJECT) {
		throw std::runtime_error("onStatus: 未找到结果对象");//결과 개체가 없습니다.
	}
	...
	if(m_dqOnStatusCB.size()) {
		...
		if(level.type() == AMF_STRING){
			if(level.as_string() != "status") {
				throw std::runtime_error(StrPrinter <<"onStatus 失败:" << level.as_string() << " " << code << endl);//onStatus가 실패합니다.
			}
		}
    }
}
