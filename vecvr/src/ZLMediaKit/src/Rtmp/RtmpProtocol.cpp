...
void RtmpProtocol::sendRtmp(uint8_t ui8Type, uint32_t ui32StreamId,
	...
	if (iChunkId < 2 || iChunkId > 63) {
		auto strErr = StrPrinter << "不支持发送该类型的块流 ID：" << iChunkId << endl;//이 유형의 블록 스트림 ID 전송은 지원되지 않습니다.
...
void RtmpProtocol::handle_S0S1S2(const function<void()> &callBack) {
	...
	//握手结束 //악수의 끝
	m_nextHandle = [this]() {
		//握手结束并且开始进入解析命令模式 //핸드 셰이크가 종료되고 구문 분석 명령 모드로 들어가기 시작합니다.
		handle_rtmp();
	};
	callBack();
}
...
void RtmpProtocol::handle_C0C1() {
	...
	if(memcmp(m_strRcvBuf.c_str() + 5,"\x00\x00\x00\x00",4) ==0 ) {
		...
		handle_C1_complex();
#else
		WarnL << "未打开ENABLE_OPENSSL宏，复杂握手采用简单方式处理！";//ENABLE_OPENSSL 매크로는 열리지 않으며 복잡한 핸드 셰이크는 간단한 방식으로 처리됩니다!
		handle_C1_simple();
#endif//ENABLE_OPENSSL
	}
...
void RtmpProtocol::handle_C1_complex() {
	//参考自：http://blog.csdn.net/win_lin/article/details/13006803 //참조
	...
	try {
		...
		//貌似flash从来都不用schema1 //플래시는 schema1을 사용하지 않는 것 같습니다.
...
void RtmpProtocol::check_C1_Digest(const string &digest,const string &data) {
	...
	if(sha256 != digest) {
		throw std::runtime_error("digest不匹配");//다이제스트가 일치하지 않습니다
	}
	else {
...
void RtmpProtocol::send_complex_S0S1S2(int schemeType,const string &digest) {
	//S1S2计算参考自：https://github.com/hitYangfei/golang/blob/master/rtmpserver.go //S1S2 계산 참조
	//发送S0 //S0 보내기
	char handshake_head = HANDSHAKE_PLAINTEXT;
	onSendRawData(&handshake_head, 1);
	//S1
...
void RtmpProtocol::handle_C2() {
	...
	//握手结束，进入命令模式 //핸드 셰이크 종료, 명령 모드 시작
...
void RtmpProtocol::handle_rtmp() {
	while (!m_strRcvBuf.empty()) {
		...
		case 0: {
			//0 值表示二字节形式，并且 ID 范围 64 - 319 //값 0은 2 바이트 형식을 나타내며 ID 범위는 64-319입니다.
			//(第二个字节 + 64)。//(두 번째 바이트 + 64).
			...
			break;
		case 1: {
			//1 值表示三字节形式，并且 ID 范围为 64 - 65599 //값 1은 3 바이트 형식을 나타내며 ID 범위는 64-65599입니다.
			//((第三个字节) * 256 + 第二个字节 + 64)。//((세 번째 바이트) * 256 + 두 번째 바이트 + 64).
			...
			break;
		default:
			//带有 2 值的块流 ID 被保留，用于下层协议控制消息和命令。//2 개의 값이있는 블록 스트림 ID는 하위 계층 프로토콜 제어 메시지 및 명령에 사용하도록 예약되어 있습니다.
			break;
		}
        ...
        if (chunkData.bodySize < chunkData.strBuf.size()) {
			throw std::runtime_error("非法的bodySize");//잘못된 bodySize
		}
