...
public:
	...
	virtual void regist() {
		//注册该源，注册后rtmp服务器才能找到该源 //소스를 등록하면 rtmp 서버가 등록 후 소스를 찾을 수 있습니다.
...
private:
	...
	RingBuffer<RtmpPacket>::Ptr m_pRing; //rtp环形缓冲 //rtp 링 버퍼
	ThreadPool &m_thPool;
	static unordered_map<string, unordered_map<string,weak_ptr<RtmpMediaSource> > > g_mapMediaSrc; //静态的媒体源表 //정적 미디어 소스 테이블
