...
public:
	H264Decoder(void){
		...
		if (!pCodec) {
			throw std::runtime_error("未找到H264解码器");//throw std :: runtime_error ( "H264 디코더를 찾을 수 없음");
		}
		...
		if (!m_pContext) {
			throw std::runtime_error("创建解码器失败");//throw std :: runtime_error ( "디코더 생성 실패");
		}
		...
		if(avcodec_open2(m_pContext.get(), pCodec, NULL)< 0){
			throw std::runtime_error("打开编码器失败");//throw std :: runtime_error ( "인코더를 열지 못했습니다.");
		}
		m_pFrame.reset(av_frame_alloc(),[](AVFrame *pFrame){
			av_frame_free(&pFrame);
		});
		if (!m_pFrame) {
			throw std::runtime_error("创建帧缓存失败");//throw std :: runtime_error ( "프레임 버퍼 생성 실패");
		}
	}
