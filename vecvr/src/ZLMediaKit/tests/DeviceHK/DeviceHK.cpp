...
DevChannelHK::DevChannelHK(int64_t i64LoginId, const char* pcDevName, int iChn, bool bMainStream) :
	...
	previewInfo.lChannel = iChn; //通道号 //채널 번호
	previewInfo.dwStreamType = bMainStream ? 0 : 1; // 码流类型，0-主码流，1-子码流，2-码流3，3-码流4 等以此类推 //스트림 유형, 0- 메인 스트림, 1- 서브 스트림, 2- 스트림 3, 3- 스트림 4 등
	previewInfo.dwLinkMode = 1; // 0：TCP方式,1：UDP方式,2：多播方式,3 - RTP方式，4-RTP/RTSP,5-RSTP/HTTP //0 : TCP 모드, 1 : UDP 모드, 2 : 멀티 캐스트 모드, 3-RTP 모드, 4-RTP / RTSP, 5-RSTP / HTTP
	previewInfo.hPlayWnd = 0; //播放窗口的句柄,为NULL表示不播放图象 //재생 창의 핸들, NULL은 재생되는 이미지가 없음을 의미합니다.
	previewInfo.byProtoType = 0; //应用层取流协议，0-私有协议，1-RTSP协议 //애플리케이션 계층 스트리밍 프로토콜, 0- 개인 프로토콜, 1-RTSP 프로토콜
	previewInfo.dwDisplayBufNum = 1; //播放库播放缓冲区最大缓冲帧数，范围1-50，置0时默认为1 //재생 라이브러리의 재생 버퍼에 버퍼링되는 최대 프레임 수, 범위는 1-50이며, 0으로 설정하면 기본값은 1입니다.
	...
	if (m_i64PreviewHandle == -1) {
		throw std::runtime_error( StrPrinter 	<< "设备[" << pcDevName << "/" << iChn << "]开始实时预览失败:"
												<< NET_DVR_GetLastError() << endl);//장비[ //] 실시간 미리보기 시작 실패 :
	}
}
...
void DevChannelHK::onPreview(DWORD dwDataType, BYTE* pBuffer, DWORD dwBufSize) {
	//TimeTicker1(-1);
	switch (dwDataType) {
	case NET_DVR_SYSHEAD: { //系统头数据 //시스템 헤더 데이터
		if (!PlayM4_GetPort(&m_iPlayHandle)) {  //获取播放库未使用的通道号 //재생 라이브러리의 사용하지 않은 채널 번호를 가져옵니다.
			WarnL << "PlayM4_GetPort:" << NET_DVR_GetLastError();
			break;
		}
		if (dwBufSize > 0) {
			if (!PlayM4_SetStreamOpenMode(m_iPlayHandle, STREAME_REALTIME)) { //设置实时流播放模式 //실시간 스트리밍 모드 설정
				WarnL << "PlayM4_SetStreamOpenMode:" << NET_DVR_GetLastError();
				break;
			}
			if (!PlayM4_OpenStream(m_iPlayHandle, pBuffer, dwBufSize,
					1024 * 1024)) {  //打开流接口 //오픈 스트림 인터페이스
			...
			if (!PlayM4_Play(m_iPlayHandle, 0)) {  //播放开始 //플레이 시작
				WarnL << "PlayM4_Play:" << NET_DVR_GetLastError();
				break;
			}
			InfoL << "设置解码器成功！" << endl; //디코더를 성공적으로 설정하십시오!
			//打开音频解码, 需要码流是复合流 //오디오 디코딩을 켜십시오. 코드 스트림은 복합 스트림이어야합니다.
		...
		break;
	case NET_DVR_STREAMDATA: { //流数据（包括复合流或音视频分开的视频流数据）//스트리밍 데이터 (오디오 및 비디오에서 분리 된 컴포지트 스트림 또는 비디오 스트림 데이터 포함)
		...
		break;
	case NET_DVR_AUDIOSTREAMDATA: { //音频数据 //오디오 데이터
	}
		break;
	case NET_DVR_PRIVATE_DATA: { //私有数据,包括智能信息 //스마트 정보를 포함한 개인 데이터
	}
		break;
	default:
		break;
