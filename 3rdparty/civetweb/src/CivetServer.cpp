...
void
CivetWebSocketHandler::handleClose(CivetServer *server, const struct mg_connection *conn)
{
	UNUSED_PARAMETER(server);
	UNUSED_PARAMETER(conn);
...
CivetServer::requestHandler(struct mg_connection *conn, void *cbdata)
{
	...
	//Debug::logger().info(request_info->remote_addr);
...
int
CivetServer::webSocketConnectionHandler(const struct mg_connection *conn, void *cbdata)
{
	const struct mg_request_info *request_info = mg_get_request_info(conn);
	assert(request_info != NULL);
	CivetServer *me = (CivetServer *)(request_info->user_data);
	...
	//handler->setHandshakeTimeout(3000);
...
void
CivetServer::webSocketCloseHandler(const struct mg_connection *conn, void *cbdata)
{
	const struct mg_request_info *request_info = mg_get_request_info(conn);
	assert(request_info != NULL);
	CivetServer *me = (CivetServer *)(request_info->user_data);
...
CivetServer::CivetServer(const char **options, const struct CivetCallbacks *_callbacks)
    : context(0)
{
...
CivetServer::CivetServer(std::vector<std::string> options, const struct CivetCallbacks *_callbacks)
    : context(0)
{
	struct CivetCallbacks callbacks;
...
void
CivetServer::addWebSocketHandler(const std::string &uri, CivetWebSocketHandler *handler)
{
...
const char *
CivetServer::getHeader(struct mg_connection *conn, const std::string &headerName)
{
