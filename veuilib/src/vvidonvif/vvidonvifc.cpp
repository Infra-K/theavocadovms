...
bool VVidOnvifC::CheckOnline(astring strUser, astring strPasswd, astring strUrl)
{
	...
	OnvifClientDevice * pClient  = new OnvifClientDevice(strUrl, strUser, strPasswd);
	...
	delete pClient;
	pClient = NULL;
	//VDC_DEBUG("%s %s %d : %s\n", __FUNCTION__, __FILE__, __LINE__, strUrl.c_str());

	return bRet;
}

bool VVidOnvifC::GetProfiles(astring strUser, astring strPasswd, astring strUrl, VVidOnvifProfileMap &pMap)
{
	bool bRet = false;
	...
	if (pClient->GetCapabilities() == SOAP_OK)
	{
		...
		if (pMedia->GetProfiles(profiles) == SOAP_OK)
		{
			...
			for (vector<tt__Profile * >::iterator iter = profiles.Profiles.begin(); iter < profiles.Profiles.end(); ++iter)
			{
...
bool VVidOnvifCPtz::Init(astring strUser, astring strPasswd, astring strUrl)
{
	...
	m_pPri->pOnvifDevice = new OnvifClientDevice(strUrl, strUser, strPasswd);
	...
	{
		VDC_DEBUG("%s %s %d : new Got profiles size %d \n", __FUNCTION__, __FILE__, __LINE__, profileMap.size());
	}
	else
	{
