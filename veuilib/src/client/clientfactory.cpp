...
BOOL ClientFactory::Init()
{
	...
	//R_LOG(logRINFO, "%s %s %d %s\n", __FUNCTION__, __FILE__, __LINE__, strConf.c_str());

	/* Init export path */
	astring strExportPath;
	GetExportPath(strExportPath);

	//R_LOG(logRINFO, "%s %s %d %s %s\n", __FUNCTION__, __FILE__, __LINE__, strConf.c_str(), strExportPath.c_str());
...
BOOL ClientFactory::GetLicense(astring &strLicense, astring &strHostId, int &ch, astring &type, astring &startTime, astring &expireTime)
{
	XGuard guard(m_cMutex);
