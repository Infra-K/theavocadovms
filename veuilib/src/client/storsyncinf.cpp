...
#ifdef AVOCADO_DEBUG
	if (strstr(data, "LINK_CMD_SEARCH_RECORD_RESP") == NULL && strstr(data, "LINK_CMD_HAS_RECORD_RESP") == NULL)
		VDC_DEBUG("%s %s %d data:%s, type:%d\n", __FUNCTION__, __FILE__, __LINE__, data, cmd.type());
#endif
...
{
#ifdef AVOCADO_DEBUG
	VDC_DEBUG("%s %s %d strId=%s\n", __FUNCTION__, __FILE__, __LINE__, strId.c_str());
#endif
...
bool StorSyncInf::GetLic(astring &pLic, astring &strHostId, int &ch, astring &type, astring &startTime, astring &expireTime)
{
	if (Connect() == false)
	{
#ifdef AVOCADO_DEBUG
...
bool StorSyncInf::CamSearchGet(astring &strIP, astring &strPort, astring &strModel, astring &strOnvifAddr)
{
	XGuard guard(m_cMutex);
