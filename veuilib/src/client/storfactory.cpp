...
BOOL StorFactory::Init()
{
	...
	for (s32 i = 0; i < storList.cvidstor_size(); i ++)
	{
		...
		InitAddStor(pStor);
		// VDC_DEBUG("%s  %s %d %s\n", __FUNCTION__, __FILE__, __LINE__, pStor.strid().c_str());
	}
...
bool StorFactory::AddStor(VidStor & pParam)
{
	...
	if (m_pConf.FindStor(pParam.strid()))
	{
		...
		delete m_StorClientMap[pParam.strid()];//점검필요
...
bool StorFactory::DeleteStor(astring strId)
{
	...
	if (m_pConf.FindStor(strId))
	{
		...
		//delete m_StorClientMap[strId];점검필요
...
{	
	...
	if (bRet == true)
	{
		VDC_DEBUG("%s Get Event %d %s %s bsearched %d\n", __FUNCTION__, 
					pEvent.strtime().c_str(), pEvent.strdevicename().c_str(), pEvent.strtype().c_str(), pEvent.bsearched());
