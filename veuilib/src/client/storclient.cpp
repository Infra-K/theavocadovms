...
:m_stor(stor), m_pNotify(pNotify), m_Quit(false), StorWebSocketClient(stor.strip(), stor.strport(), LINK_PROTO_WS_PATH)
{
		...
		case Link::LINK_CMD_CAM_DEL_NOTIFY:
		{
			...
			if (cmd.has_camidnotify())
			{
				...
				for (s32 i = 0; i < m_cCamList.cvidcamera_size(); i ++)
				{
					...
					{
						VidCamera *pNewCam = cameraListNew.add_cvidcamera();
						*pNewCam = cam;
					}
				}
