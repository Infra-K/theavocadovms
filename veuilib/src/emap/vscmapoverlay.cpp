...
VSCMapOverlay::VSCMapOverlay(QWidget *parent) : QWidget(parent), d(new VSCMapOverlayPrivate(this))
{
    updatePosition();
}
...
{
    if(windowSz.height() < drawingRect.height() || windowSz.width() < drawingRect.width()) {
        this->show();
        d->outerRect.setX(0);
        d->outerRect.setY(0);
        ...
        d->outerRect.translate(d->mapSz - d->outerRect.width(), d->mapSz - d->outerRect.height());
        d->innerRect.translate(d->mapSz - d->outerRect.width(), d->mapSz - d->outerRect.height());
        update();
    }
    else {
        this->hide();
