...
VSCImageViewer::VSCImageViewer(QWidget* parent, QImage& image) : QWidget(parent), d(new VSCImageViewerPrivate(this))
{
    setImage(image);
}
...
void VSCImageViewer::fitAll()
{
    ...
    if(h && w) {
        ...
        float drawingAspect = (float) d->drawingRect.height() / d->drawingRect.width();
