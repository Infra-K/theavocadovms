 ...
 : m_cCam(cCam), m_pFactory(pFactory), QWidget(parent, flags)
{
	...
	astring strName = m_pFactory.GetStorFactory().GetVidCameraName(m_cCam.cid().strstorid(), m_cCam.cid().strcameraid());
...
{
	QMimeData *mimeData = new VSCQMimeVidCam(m_cCam.cid().strstorid(), m_cCam.cid().strcameraid(), "Camera");
