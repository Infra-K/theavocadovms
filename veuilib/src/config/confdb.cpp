...
s32 ConfDB::Open(astring  pPath)
{
	...
	if (false == status.ok())
	{
		...
		VDC_DEBUG("%s %s %d Unable to open/create test database %s\n", __FUNCTION__, __FILE__, __LINE__, pPath.c_str());
...
BOOL ConfDB::GetLicense(astring &strLicense)
{
	...
	leveldb::Status s = m_pDb->Get(leveldb::ReadOptions(), key, &strLicense);
	if (s.ok() == false)
	{
		strLicense = "";
...
BOOL ConfDB::GetCmnParam(astring &strKey, astring &strParam)
{
	...
	leveldb::Status s = m_pDb->Get(leveldb::ReadOptions(), key, &strParam);
	if (s.ok() == false)
	{
		strParam = "";
...
BOOL ConfDB::GetHdfsRecordConf(VidHDFSConf &pData)
{
	...
	leveldb::Status s = m_pDb->Get(leveldb::ReadOptions(), key, &strValue);
	if (s.ok() == false)
	{
		strValue = "fake";
	}
	...
	{
		VDC_DEBUG("%s %s %d Hdfs Record Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
...
BOOL ConfDB::GetCameraListConf(VidCameraList &pData)
{
	...
	if (pData.ParseFromString(strValue) == false)
	{
		...
		VDC_DEBUG("%s %s %d Camera List Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
...
bool ConfDB::GetStorServerConf(VidStorServerConf &pData)
{
	...
	if (pData.ParseFromString(strValue) == false)
	{
		...
		VDC_DEBUG("%s %s %d Stor Server Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
...
BOOL ConfDB::GetSystemConf(VSCConfData &pSys)
{
    ...
    {
		VDC_DEBUG( "%s %s %d System Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
        delete it;
...
BOOL ConfDB::GetVmsConf(VSCVmsData &pVms)
{
    ...
    {
		VDC_DEBUG( "%s %s %d VMS Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
        delete it;
...
BOOL ConfDB::GetViewConf(VSCViewData &pView)
{
    ...
    {
		VDC_DEBUG( "%s %s %d View Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
        delete it;
...
BOOL ConfDB::GetVGroupConf(VSCVGroupData &pVGroup)
{
    ...
    {
		VDC_DEBUG( "%s %s %d Group Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
        delete it;
...
BOOL ConfDB::GetUserConf(VSCUserData &pData)
{
    ...
    {
		VDC_DEBUG( "%s %s %d User Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
        delete it;
...
BOOL ConfDB::GetTourConf(VSCTourData &pData)
{
    ...
    {
		VDC_DEBUG( "%s %s %d Tour Config is not init\n", __FUNCTION__, __FILE__, __LINE__);
        delete it;
...
BOOL ConfDB::GetDeviceData(u32 nId, VSCDeviceData &pData)
{
    ...
    {
		VDC_DEBUG( "%s %s %d Device Can not find !!!\n", __FUNCTION__, __FILE__, __LINE__);
