...
SmartMotionCam::SmartMotionCam(string strId, int nStreamId, astring strName,  astring strMotReg, Factory &pFactory, VEventServer &pEventServer)
 : m_strId(strId), m_nStreamId(nStreamId), m_pFactory(pFactory), m_MotAlgo(false), m_pEventServer(pEventServer), m_nLastMotTime(0), m_strName(strName), 
   m_motReg(VVidMotReg::CreateAll())
{
	...
	m_vPlay.StartGetRawFrame(this, (VPlayRawFrameHandler)SmartMotionCam::RawHandler);
	if (m_nStreamId == 1)
	{
		m_pFactory.RegDataCallback(m_strId,
		(CameraDataCallbackFunctionPtr)SmartMotionCam::DataHandler, (void *)this);
	}
...
void SmartMotionCam::RawHandler1(RawFrame& frame)
{
	...
	{	
#ifdef AVOCADO_DEBUG
		VDC_DEBUG("%s %s %d Smart Motion Got\n", __FUNCTION__, __FILE__, __LINE__);
#endif
		astring strEvtTime;
...
bool VSmartMotionThread::DeviceChangeCallbackFunc(void* pData, FactoryCameraChangeData change)
{
	// 너무 많이 나옴
	if (pData)
	{
...
void VSmartMotionThread::run()
{
	...
	m_pFactory.RegCameraChangeNotify((void *)this, (FactoryCameraChangeNotify)(VSmartMotionThread::DeviceChangeCallbackFunc));	
...
#endif
	SmartMotionCam *pMotCam = new SmartMotionCam(pParam.strid(), pParam.nservermotionstream(), pParam.strname(), pParam.strmotreg(), m_pFactory, m_pEventServer);
