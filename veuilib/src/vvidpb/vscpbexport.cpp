...
VSCPbExportTask::VSCPbExportTask(astring strExportPath, astring strCamName, VidStor &pStor, astring strId, u32 nPlaytime, u32 nEndTime)
	...
	m_isEol(FALSE), m_pMuxer(NULL), m_bQuit(false)
{
	...
	astring strFileName =m_strCamName + " " + VSCPbExportTaskTime2String(nPlaytime) + "-" + VSCPbExportTaskTime2String(nEndTime) + ".mp4";
...
void VSCPbExportTask::PBCallbackHandler1(VideoFrame& frame)
{
	...
	{
		VDC_DEBUG("VIDEO_STREAM_INFO is NULL\n");
		return;
	}
...
std::string VSCPbExportTask::GetTaskName()
{
	...
	strName = tr("Record export  ") + strDeviceName + "  ( " + m_qStartTime.toString(Qt::ISODate) + tr(" to ") + m_qEndTime.toString(Qt::ISODate) + " )";
