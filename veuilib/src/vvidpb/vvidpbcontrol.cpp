...
VVidPBControl::VVidPBControl(QWidget *parent, Qt::WindowFlags flags)
 : p_ui(new Ui::VVidPBControl), ui(*p_ui), QWidget(parent, flags), m_pDateSetting(NULL), m_pExport(NULL), m_bPause(false)
{
	ui.setupUi(this);
	...
	connect(ui.timeline, SIGNAL(SignalSeekTime(unsigned int)), this, SLOT(SlotSeekTime(unsigned int)));
	...
	connect(this, SIGNAL(SignalTimeUpdate(unsigned int)), ui.timeline, SLOT(SlotUpdateTime(unsigned int)));
...
bool VVidPBControl::SetDelegate(VVidPBDelegate *pDelegate)
{
	...
	if (m_pExport == NULL)
	{
		m_pExport = new VSCExport(m_strStor, m_strId, m_strCamName, ui.timeline->GetCurrentTime(), this);
	}
...
void VVidPBControl::SlotExport()
{
	...
	m_pExport->move(pos.x() - m_pExport->width(), pos.y()- m_pExport->height());
...
void VVidPBControl::SlotDateSetting()
{
	...
	QPoint pos = mapTo(this, QCursor::pos());
	m_pDateSetting->move(pos.x() - m_pDateSetting->width(), pos.y()- m_pDateSetting->height());
	m_pDateSetting->show();
}
