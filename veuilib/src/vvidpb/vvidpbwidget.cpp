...
VVidPBWidgetDelegate::VVidPBWidgetDelegate(ClientFactory &pFactory, VidStor &stor, string strId, astring strCamName)
...
bool VVidPBWidgetDelegate::SearchRec(u32 nStart, u32 nEnd, RecordItemMap &pMap)
{
...
VVidPBWidget::VVidPBWidget(VVidPBControl &pControl, ClientFactory &pFactory, s32 nId, QWidget *parent, bool bSingle, Qt::WindowFlags flags)
 : m_pControl(pControl), m_pFactory(pFactory), m_bSingle(bSingle), QWidget(parent, flags), p_ui(new Ui::VVidPBWidget), ui(*p_ui), m_StorStream(NULL), m_Delegate(NULL)
{
	m_pStarted = FALSE;
	m_nId = nId;
...
#ifdef AVOCADO_DEBUG
	// 개수만큼 잘나옴
	// VDC_DEBUG("%s %s %d ~VVidPBWidget id %d\n", __FUNCTION__, __FILE__, __LINE__, m_nId);
#endif
	if (m_pStarted == true)
	{
...
{
#ifdef AVOCADO_DEBUG
	VDC_DEBUG("%s %s %d Enter in dropEvent id %d\n", __FUNCTION__, __FILE__, __LINE__, m_nId);
#endif
    event->accept();
}
...
void VVidPBWidget::dragMoveEvent(QDragMoveEvent *event)
{
	...
	//많이 나옴
	//VDC_DEBUG("%s %s %d Enter in dropEvent id %d\n", __FUNCTION__, __FILE__, __LINE__, m_nId);
	//VVidPBWidget::dragMoveEvent ..\src\vvidpb\vvidpbwidget.cpp 192 Enter in dropEvent id 0
#endif
    event->accept();
}
...
void VVidPBWidget::dropEvent(QDropEvent *event)
{
	...
	if (pVidCam)
	{
		...
		StartPlay(pVidCam->m_strStorId, pVidCam->m_strCamId, pVidCam->m_strCamName, currentTime);
	...
	if (pWinSwitch)
	{
		...
		{
	#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d Enter in dropEvent Same with me %d\n", __FUNCTION__, __FILE__, __LINE__, pWinSwitch->m_nDstId);
	#endif
			return;
		}
...
void VVidPBWidget::videoResizeEventTimer()
{
	...
	if (m_pStarted == TRUE && m_StorStream)
	{
		...
		{
			m_StorStream->UpdateWidget(m_videoWindow, ui.video->width(), ui.video->height());
		}
		else
		{
			return;
...
void VVidPBWidget::SlotTimerInfoUpdate()
{
	if (m_pStarted == TRUE)
	{
		...
		strName =  m_strCamName + " ["  + strCodec + " " + strWidth + " " + "x" + " " + strHeight + "]";
...
bool VVidPBWidget::SetupPlayConnection()
{
	...
	connect(m_StorStream, SIGNAL(SignalPlayTime(unsigned int)), &m_pControl, SLOT(SlotUpdateTime(unsigned int)));
	/* Control to Stream */
	connect(&m_pControl, SIGNAL(SignalSeekTime(unsigned int)), this, SLOT(SlotSeekTime(unsigned int)));	
...
BOOL VVidPBWidget::StartPlay(astring strStorId, astring strCamId, astring strCamName, u32 nPlaytime)
{
	m_pStarted = TRUE;
...
BOOL VVidPBWidget::StopPlay()
{
	if (m_pStarted == true)
	{
		...
		m_pControl.SetPlayParam(VVID_UUID_NULL, VVID_UUID_NULL, VVID_UUID_NULL);
		m_StorStream->DetachWidget(m_videoWindow);
		m_StorStream->StopStorStream();
		delete m_StorStream;
		m_StorStream = NULL;
...
{
#ifdef AVOCADO_DEBUG
	VDC_DEBUG("%s %s %d mousePressEvent %d\n", __FUNCTION__, __FILE__, __LINE__, m_nId);
#endif
    ...
    if (m_pStarted == TRUE)
    {
        if (m_DragStart == FALSE)
        {
			m_lastDragX = e->x();
		#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d mousePressEvent %d start x %d\n", __FUNCTION__, __FILE__, __LINE__, m_nId, m_lastDragX);
		#endif
			m_DragStart = TRUE;
			e->accept();
			return;
...
{
#ifdef AVOCADO_DEBUG
	VDC_DEBUG("%s %s %d, id=%d mouseReleaseEvent \n", __FUNCTION__, __FILE__, __LINE__, m_nId);
#endif
...
void VVidPBWidget::videoMouseMove(QMouseEvent *e)
{
    ...
    if (m_pStarted == TRUE)
    {
        ...
        {
		#ifdef AVOCADO_DEBUG
			VDC_DEBUG("%s %s %d mouseMoveEvent %d event x %d\n", __FUNCTION__, __FILE__, __LINE__, m_nId, e->x());
		#endif
...
{
#ifdef AVOCADO_DEBUG
	VDC_DEBUG("%s %s %d mouseDoubleClickEvent %d\n", __FUNCTION__, __FILE__, __LINE__, m_nId);
#endif
