...
BOOL VEventServerSearchTask::ProcessSearchCmd(VVidEventSearchCmd &pCmd)
{
	...
	while (1)
	{
	    ...
	    Statement stmt = (*m_DB << "SELECT id FROM events WHERE strDevice=:strDevice AND id >:recordid AND (nTime>=:starttime AND  nTime<=:endtime)", 
									use(pCmd.strId), use(recordLastId), use(pCmd.nStart), use(pCmd.nEnd), into(recordIds), limit(1));
	    stmt.execute();

		VDC_DEBUG("%s %s %d recordLastId %d \n", __FUNCTION__, __FILE__, __LINE__, recordLastId);
	...
	#if 1
			*m_DB << "SELECT strId FROM events WHERE id=:id", use(recordId), into(sEvent.strId), now;
			*m_DB << "SELECT strDeviceName FROM events WHERE id=:id", use(recordId), into(sEvent.strDeviceName), now;
			*m_DB << "SELECT strType FROM events WHERE id=:id", use(recordId), into(sEvent.strType), now;
			*m_DB << "SELECT nTime FROM events WHERE id=:id", use(recordId), into(sEvent.nTime), now;
			*m_DB << "SELECT strEvttime FROM events WHERE id=:id", use(recordId), into(sEvent.strEvttime), now;
			*m_DB << "SELECT strDesc FROM events WHERE id=:id", use(recordId), into(sEvent.strDesc), now;
			*m_DB << "SELECT bHandled FROM events WHERE id=:id", use(recordId), into(sEvent.bHandled), now;
			*m_DB << "SELECT strComments FROM events WHERE id=:id", use(recordId), into(sEvent.strComments), now;
	#else
...
BOOL VEventServerSearchTask::ProcessHandleCmd(VVidEventSearchCmd &pCmd)
{
	...
	/* Set handle true */
	*m_DB << "UPDATE events SET bHandled=:bHandled WHERE strId=:strId", use(bHandled), use(pCmd.strId), now;
...
void VEventServerDbTask::UpdateDBSession(bool bIsFirst)
{
	...
	if (cDBConf.ntype() == VID_DB_SQLITE)
	{
		...
		{	
			...
			VDC_DEBUG("%s %s %d Create the Event DB path %s\n", __FUNCTION__, __FILE__, __LINE__, path.toString().c_str());
			
			Poco::Path filePath("eventdb.db");
			path.append(filePath);
			path.makeFile();

			VDC_DEBUG("%s %s %d Create the Event DB file %s\n", __FUNCTION__, __FILE__, __LINE__, path.toString().c_str());
		...
		{			
			VDC_DEBUG("%s %s %d Create SQLITE session failed \n", __FUNCTION__, __FILE__, __LINE__);
		}
	}
	else
	{
		VDC_DEBUG("%s %s %d Do not support the DB TYPE \n", __FUNCTION__, __FILE__, __LINE__);
		ve_sleep(1000);
	}	
}
...
void VEventServerDbTask::run()
{
	...
	{		
		...
		if (m_pSqlSession == NULL)
		{
			VDC_DEBUG("%s %s %d Database session is NULL \n", __FUNCTION__, __FILE__, __LINE__);
			continue;
		}
		...
		try
		{
			...
			*m_pSqlSession << "INSERT INTO events VALUES(NULL, :strId, :strDevice, :strDeviceName, :strType, :nTime, :strEvttime, :strDesc, :bHandled, :strComments)", 
		...
		{			
			VDC_DEBUG("%s %s %d insert event error \n", __FUNCTION__, __FILE__, __LINE__);
		}
	}
}

VEventServer::VEventServer(Factory &pFactory)
 : m_Factory(pFactory), m_DbTask(pFactory), m_CallbackTask(pFactory), m_SearchTask(pFactory, m_DbTask)
{
