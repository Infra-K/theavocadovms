...
VMotTable::VMotTable(QWidget *parent, Qt::WindowFlags flags)
 : QWidget(parent, flags), m_pPri(NULL), m_bPressed(false)
{
	...
	connect(m_pPri->pNetwManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(SlotNetowkManagerFinished(QNetworkReply*)));
}
...
void VMotTable::paintEvent(QPaintEvent *e)
{
	...
	for (int i = 0; i < VVID_MOT_REGION_H; i ++)
	{
		for (int j = 0; j < VVID_MOT_REGION_W; j ++)
		{
            ...
            painter.drawRect(widthPerW * (double)j, heightPerH * (double)i,	widthPerW - 1, heightPerH - 1);
		}
	}
...
void VMotTable::mouseMoveEvent(QMouseEvent *e)
{
	...
	m_pPri->pRubberBand->setGeometry(m_pPri->nStartX, m_pPri->nStartY, pos.x()-m_pPri->nStartX, pos.y()-m_pPri->nStartY);
	m_pPri->pRubberBand->show();	
}
...
bool VMotTable::UpdateMotType(bool type)
{
	...
	for (int x = nStartX; x <= nStartX + m_pPri->pRubberBand->width()/widthPerW; x++)
	{
		for (int y = nStartY; y <= nStartY + m_pPri->pRubberBand->height()/heightPerH; y++)
		{
