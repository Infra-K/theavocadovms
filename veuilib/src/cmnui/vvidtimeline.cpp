...
void TimeLineGrid::drawMarks(QPainter *painter)
{
    ...
    drawCurrTimeMark(msecPerPixel, currTime, startTime, endTime, currTimeMarkBorders, mDayTimeFormat, fm, painter);
...
{
	connect(&m_pThread, SIGNAL(RecItemUpdated()), this, SLOT(SlotUpdateSearch()), Qt::QueuedConnection);
	m_pThread.start();	
}
...
void TimeLineItems::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	...
	for(; it!=m_RecItems.end(); ++it)
	{
		...
		/* Draw the item */
		if (item.start < startTime && item.end > startTime)
		{
			//34, 139, 34
			painter->fillRect(0, 0, (double)(item.end - startTime) * rate + 1, mSize.height(), QBrush(color));
		}
			...
			//&& item.end <= endTime)
		{
			...
			painter->fillRect(startPos, 0, (double)(item.end - item.start) * rate + 1, mSize.height(), QBrush(color));
		}
	}	
}
...
{
    mScheduledScaling = mScheduledScaling > 0 ? mScheduledScaling - 1 : mScheduledScaling + 1;
}
...
void VVIDTimeLineWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    if (rect().contains(event->pos()) && event->button() == Qt::LeftButton) // otherwise scroll the timeline
    {
