...
void VSchedTable::paintEvent(QPaintEvent *e)
{
	...
	for (int i = 1; i <= VDB_DAYS_IN_WEEK; i ++)
	{
		for (int j = 1; j <= VDB_HOURS_IN_DAY; j ++)
		{
			...
			painter.drawRect(widthPerHour * j + 1, heightPerDay * i  + 1, widthPerHour - 2, heightPerDay - 2);
		}
	}
...
bool VSchedTable::UpdateSchedType(RecordingType type)
{
	...
	for (int x = nStartX; x <= nStartX + m_pPri->pRubberBand->width()/widthPerHour; x ++)
	{
		for (int y = nStartY; y <= nStartY + m_pPri->pRubberBand->height()/heightPerDay; y ++)
		{
