...
: m_pFactory(pFactory), m_bFloated(FALSE), QWidget(parent)
{
    ...
    for (int i = 0; i < VIDEO_WALL_WIDGET_MAX; i ++)
    {
        ...
        connect(m_VideoMap[i], SIGNAL(ShowDisplayClicked(int)), this, SIGNAL(ShowDisplayClicked(int)));
        connect(m_VideoMap[i], SIGNAL(ShowFloatingClicked()), this, SIGNAL(ShowFloatingClicked()));
        connect(m_VideoMap[i], SIGNAL(ShowTabbedClicked()), this, SIGNAL(ShowTabbedClicked()));
        connect(m_VideoMap[i], SIGNAL(ShowControlPanelClicked()), this, SIGNAL(ShowControlPanelClicked()));
        ...
        connect(m_VideoMap[i], SIGNAL(ShowViewClicked(std::string)), this, SIGNAL(ShowViewClicked(std::string))); 
        connect(m_VideoMap[i], SIGNAL(PlaybackClicked(std::string, std::string, std::string)), this, SIGNAL(PlaybackClicked(std::string, std::string, std::string)));
        connect(m_VideoMap[i], SIGNAL(MotionDetectClicked(std::string, std::string, std::string)), this, SIGNAL(MotionDetectClicked(std::string, std::string, std::string)));
        connect(m_VideoMap[i], SIGNAL(SignalUpdateMainView()), this, SIGNAL(SignalUpdateMainView()));
    }
...
void VSCVideoWall::ClearVideoLayout()
{
    ...
    for(int j=0; j<rows; j++)
    {
        for(int k=0; k<colums && i<count; k++)
        {
			...
			//VSCVideoWall::ClearVideoLayout ..\src\vvidwidget\vscvideowall.cpp 159 Removing item at : 0 0
			//VSCVideoWall::ClearVideoLayout ..\src\vvidwidget\vscvideowall.cpp 159 Removing item at : 0 1
			//  ...
			//VSCVideoWall::ClearVideoLayout ..\src\vvidwidget\vscvideowall.cpp 159 Removing item at : 3 2
			//VSCVideoWall::ClearVideoLayout ..\src\vvidwidget\vscvideowall.cpp 159 Removing item at : 3 3
...
void VSCVideoWall::PlayVideoByWindow(u32 nWindow, astring strStor, astring strCam, astring strCamName)
{
