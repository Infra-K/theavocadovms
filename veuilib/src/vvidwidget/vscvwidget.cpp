...
VSCVWidget::VSCVWidget(ClientFactory &pFactory, s32 nId, QWidget *parent, bool bSingle, Qt::WindowFlags flags, VVidGetHWDec *pGetHWDec)
 : m_pFactory(pFactory), m_bSingle(bSingle), QWidget(parent, flags), p_ui(new Ui::VSCVWidget), ui(*p_ui), m_StorStream(NULL), m_pGetHWDec(pGetHWDec), m_bHWAccel(false)
{
	m_pStarted = FALSE;
	m_nId = nId;	
	m_bFocus = FALSE;
...
VSCVWidget::~VSCVWidget()
{
	...
	//VSCVWidget::~VSCVWidget ~VSCVWidget id 0
	//VSCVWidget::~VSCVWidget ~VSCVWidget id 1
	//...
	//VSCVWidget::~VSCVWidget ~VSCVWidget id 34
	//VSCVWidget::~VSCVWidget ~VSCVWidget id 35	
	StopPlay();
...
void VSCVWidget::dragEnterEvent(QDragEnterEvent *event)
{
	...
	//VSCVWidget::dragEnterEvent Enter in dropEvent id 0
...
{
    //VDC_DEBUG("%s Enter in dropEvent id %d\n", __FUNCTION__, m_nId);
	//VSCVWidget::dragMoveEvent Enter in dropEvent id 0
...
BOOL VSCVWidget::StartPlay(astring strStorId, astring strCamId, astring strCamName, bool bMotion, bool bByView, bool bWithHWAccel)
{
	// 실시간 화면 한개의 비디오를 플레이 시작함.
	...
	{
		VDC_DEBUG("%s %s %d, StorId=%s, CamId=%s\n", __FUNCTION__, __FILE__, __LINE__, strStorId.c_str(), strCamId.c_str());
		return false;
	}
	...
	VDC_DEBUG("%s %s %d StorId=%s, CamId=%s, Name=%s\n", __FUNCTION__, __FILE__, __LINE__, strStorId.c_str(), strCamId.c_str(), strName.c_str());
...
BOOL VSCVWidget::StopPlay()
{
	...
	{
#ifdef AVOCADO_DEBUG
	VDC_DEBUG("%s %s %d, StopPlay begin\n", __FUNCTION__, __FILE__, __LINE__);
#endif
		m_TimerInfo->stop();
...
void VSCVWidget::mousePressEvent(QMouseEvent *e)
{
	...
	if (m_PtzEnable == true)
	{
		...
		m_pFactory.GetStorFactory().PtzCmd(m_strStor, m_strCam, nAction, speed);
		return;
	}
...
void VSCVWidget::wheelEvent ( QWheelEvent * event )
{
	...
	if(scale > 0)
	{
		m_pFactory.GetStorFactory().PtzCmd(m_strStor, m_strCam,	F_PTZ_ZOOM_IN, 0.2); // TODO Change//speed is 0.0 to 1.0
	}
	else
	{
		m_pFactory.GetStorFactory().PtzCmd(m_strStor, m_strCam,	F_PTZ_ZOOM_OUT, 0.2); // TODO Change//speed is 0.0 to 1.0
	}
#ifdef WIN32
