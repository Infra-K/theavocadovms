...
/// value of the variable local to each thread (known as thread-local storage, or TLS). Example usage:
/// @code
...
/// This namespace is more or less equivalent to the @c std namespace for the C++11 thread classes.
/// For instance, the tthread::mutex class corresponds to the std::mutex class.
namespace tthread {
/// Mutex class.
/// This is a mutual exclusion object for synchronizing access to shared memory areas for several threads.
/// The mutex is non-recursive (i.e. a program may deadlock if the thread that owns a mutex object calls lock() on that object).
/// @see recursive_mutex
...
public:
	...
	/// Lock the mutex.
	/// The method will block the calling thread until a lock on the mutex can be obtained.
	/// The mutex remains locked until @c unlock() is called.
	/// @see lock_guard
	inline void lock()
	{
	#if defined(_TTHREAD_WIN32_)
	...
	/// @return @c true if the lock was acquired, or @c false if the lock could not be acquired.
...
/// This is a mutual exclusion object for synchronizing access to shared memory areas for several threads.
/// The mutex is recursive (i.e. a thread may lock the mutex several times, as long as it unlocks the mutex the same number of times).
/// @see mutex
...
/// so the mutex will automatically be unlocked when the lock guard goes out of scope.
/// Example usage:
/// @code
/// mutex m;
/// int counter;
...
public:
	...
	/// The function will block the calling thread until the condition variable is woken by @c notify_one(), @c notify_all() or a spurious wake up.
	/// @param[in] aMutex A mutex that will be unlocked when the wait operation starts, an locked again as soon as the wait operation is finished.
	...
	/// @note Only threads that started waiting prior to this call will be woken up.
#if defined(_TTHREAD_WIN32_)
	void notify_one();
#else
...
public:
	...
	/// Default constructor.
	/// Construct a @c thread object without an associated thread of execution (i.e. non-joinable).
	...
	/// @note This constructor is not fully compatible with the standard C++ thread class.
	/// It is more similar to the pthread_create() (POSIX) and CreateThread() (Windows) functions.
	thread(void (*aFunction)(void *), void * aArg);

	/// Destructor.
	/// @note If the thread is joinable upon destruction, @c std::terminate() will be called, which terminates the process.
	/// It is always wise to do @c join() before deleting a thread object.
	~thread();

	/// Wait for the thread to finish (join execution flows).
	/// After calling @c join(), the thread object is no longer associated with a thread of execution
	/// (i.e. it is not joinable, and you may not join with it nor detach from it).
	void join();
	...
	/// After calling @c detach(), the thread object is no longer assicated with a thread of execution (i.e. it is not joinable).
	/// The thread continues execution without the calling thread blocking, and when the thread ends execution, any owned resources are released.
	void detach();
...
/// Minimal implementation of the @c ratio class. This class provides enough functionality to implement some basic @c chrono classes.
...
namespace chrono {
	/// Duration template class. This class provides enough functionality to implement @c this_thread::sleep_for().
...
namespace this_thread {
	...
	/// @endcode
	/// @note Supported duration types are: nanoseconds, microseconds, milliseconds, seconds, minutes and hours.
