...
VE_LIBRARY_API unsigned UtilityGetBufferSize(int bufOptName, int socket);
...
VE_LIBRARY_API unsigned UtilitySetBufferTo(int bufOptName, int socket, unsigned requestedSize);
...
VE_LIBRARY_API void CALLBACK ApcProc(ULONG_PTR);
#endif
...
VE_LIBRARY_API int uriParserGetParaByName(const char * uri, const char * name, char *out, const int out_len);
