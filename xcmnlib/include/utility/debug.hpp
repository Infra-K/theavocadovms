...
public:
	...
	:m_nPort(nPort), m_strLoggerPath(strLoggerPath)
	{
		...
		m_pLogger = spd::rotating_logger_mt("avocadovms", m_strLoggerPath, 1024 * 1024 * 5, 3, true);
	}
...
public:
	static int test(struct cli_def *cli, const char *command, char *argv[], int argc);
