...
BOOL RapidMediaData::StartRawHandlerThread()
{
	...
	/* start the data handler thread */
	this->m_pThreadRaw = new tthread::thread(RapidMediaData::RunRaw, (void *)this);
...
void RapidMediaData::RapidRTSPEventHandle(int eventType, int lParam, void *rParam, void *context)
{
	if (context)
	{
		RapidMediaData *pData = (RapidMediaData *)context;
		return pData->RapidRTSPEventHandle1(eventType, lParam, rParam);
	}
}
											...
											VideoFrameType frameType, unsigned int flag, CRapidRTSPAVInfo &pAVInfo, void *context)
{
	...
	{
		RapidMediaData *pData = (RapidMediaData *)context;
		return pData->RapidRTSPDataHandle1(pdata, dataSize, sec, msec, dataType, streamType, frameType, flag, pAVInfo);
	}
}
	...
	VideoFrameType frameType, unsigned int flag, CRapidRTSPAVInfo &pAVInfo)
{
	...
	RRLOG(RRLOG_INFO, "%s data %d size:%u timestamp (%u, %d) frametype %d flag 0x%x", __FUNCTION__, dataType, dataSize, sec, msec, frameType, flag);
#endif
	//printf("%s data %d (size:%u) (%d, %d) frametype %d flag 0x%x\n", __FUNCTION__, dataType, dataSize, sec, msec, frameType, flag);
	...
	if (skipData != TRUE && m_pThreadData)
	{
		if(m_Queue4DataFree.NonBlockingPeek() == true)
		{
			...
			if (pFrame)
			{
				...
				{
					memcpy(pFrame->dataBuf + sizeof(InfoFrameI) + padding, pdata, dataSize);
					if (padding > 0)
					{
						memcpy(pFrame->dataBuf + sizeof(InfoFrameI), pad, padding);
					}
				...
				{
					memcpy(pFrame->dataBuf + sizeof(InfoFrameP), pdata, dataSize);
...
void RapidMediaData::RapidRTSPEventHandle1(int eventType, int lParam, void *rParam)
{

}

BOOL RapidMediaData::Init(BOOL bRealStream, string strUrl, string strUser, string strPass, BOOL bHWAccel, VSCConnectType connectType)
{
...
BOOL RapidMediaData::StartRapidRTSP()
{
	...
	m_pRTSP = new CRapidRTSPLive555(m_strUrl, (int)m_connectType, m_strUser, m_strPass, true);

	m_pRTSP->set_data_handle(RapidMediaData::RapidRTSPDataHandle, (void *)this);
