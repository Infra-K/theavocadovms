...
RapidFFMPEGDec::RapidFFMPEGDec(const struct codec_options& options, enum AVCodecID id, int decodeAttempts) 
...
BOOL RapidFFMPEGDec::Decode(uint8_t* pBuf, int nSize, RawFrame & pFrame)
{
	...
	do
	{
	    ret = avcodec_decode_video2(m_context, m_frame, &gotPicture, &inputPacket);
	    attempts++;
