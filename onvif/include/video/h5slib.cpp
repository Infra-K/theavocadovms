...
bool H5SLibRTSP::onParameter(std::string strKey, std::string strValue)
{
	...
	{
		printf("%s %s %d key size 0\n", __FUNCTION__, __FILE__, __LINE__);
		return false;
	}
	printf("%s %s %d key %s value %s\n", __FUNCTION__, __FILE__, __LINE__, strKey.c_str(), strValue.c_str());
...
bool H5SLibRTSP::onData(unsigned char* buffer, int size, unsigned long long secs, unsigned long long msecs, Live555CodecType codec)
{
	//std::cout << "onData" << "  " << codec << "  " << "  " << secs << "  " << msecs  << "  " << size << std::endl;

	/* Just callback none H264 & H265 & MPEG4 */
	if (codec != LIVE555_CODEC_H264 && codec != LIVE555_CODEC_H265 && codec != LIVE555_CODEC_MPEG4)
	{	
		...
		onH5SData(buffer, size, secs, msecs, (H5SCodecType)codec, streamType, H5S_FRM_I);
		...
		case H5S_CODEC_H264:
		{
			...
			for(; it!=m_pParamMap.end(); ++it)
			{
				if ((*it).first == "sprop-parameter-sets")
				{
					...
					sPropRecords = parseSPropParameterSets((*it).second.c_str(), numSPropRecords);
...
bool H5SLibRTSP::onH5SData(unsigned char* buffer, int size, unsigned long long secs, 
			unsigned long long msecs, H5SCodecType codec, H5SStreamType stream, H5SFrameType frm)
{
