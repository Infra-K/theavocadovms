...
{
public:
	virtual bool NewCam(astring strIP, astring strPort, astring strModel, astring strONVIFAddr){return false;}
};
...
public:
	...
	void GetIPAndPort(astring &str, astring & ip, astring & port, astring & onvifAddr)
	{
		...
		while (1)
		{
			...
			if (strGetIP.find(".") == string::npos || strGetIP.find("169") == 0)
			{
				/* IPV6 address */
				pos = p2;
				continue;
...
public:
	...
	void DisThread1()
	{
		...
		while(!m_bExit)
		{
			...
			printf("[ONVIF]: ProcessRecvMsg ip %s\n", m_strAddr.c_str());
